...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_TIZEN_B2_3G
static
#endif
struct cpufreq_governor cpufreq_gov_tizen_b2_3g= {
       .name                   = "slp",
...
static struct workqueue_struct *tizen_b2_3g_cpu_hotplug_workq;
	...
	if (cpu_hotplug_go_to_sleep_flag == 1) {
		...
		case 2 :
			if ((get_run_task(1) <= 400)
				&& (cpu_load[0] < 100)
				&& (get_avg_load_freq(300) < (max_cpu_freq * 25))) {
				need_cpu_online_num = 1;
			}
...
static void __ref cpu_sim_hotplug_work(struct work_struct *work)
{
	...
	/* This is code to prevent core control
	   If CPU2/3 is on, we will turn off CPU2/3 here.
	*/
	if (pm_qos_request(PM_QOS_CPU_ONLINE_MIN) <= 3) {
		if (cpu_online(3) == 1) {
			ret = cpu_down(3);
			if (ret < 0)
				pr_err("cpu3 down err ret=%d", ret);
		}
	}
	if (pm_qos_request(PM_QOS_CPU_ONLINE_MIN) <= 2) {
		if (cpu_online(2) == 1) {
			ret = cpu_down(2);
			if (ret < 0)
				pr_err("cpu2 down err ret=%d", ret);
		}
	}
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		cpu_load_updated[j_dbs_info->cpu] = 1;
	...
	if (this_dbs_info->cpu == 0) {
		for(j=0;j<CPU_NUM; j++) {
			...
			if ((cpu_online(j)==1) && (j_dbs_info !=NULL)) {
				...
				cpu_load[j] = j_dbs_info->prev_load;
			} else
				cpu_load_updated[j] = 0;
		}
	...
	if (!smp_processor_id() && target_cpu_online != num_online_cpus()) {
		queue_work_on(this_dbs_info->cpu, tizen_b2_3g_cpu_hotplug_workq,
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	queue_delayed_work_on(cpu, tizen_b2_3g_cpu_hotplug_workq, &dbs_info->work, delay);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	queue_delayed_work_on(dbs_info->cpu, tizen_b2_3g_cpu_hotplug_workq,
			      &dbs_info->work, delay);
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		if (dbs_enable == 1) {
			...
			dbs_tuners_ins.sampling_rate = 50000;  /* 50ms */
...
#define CPU_BOOTING_TIME	17
static struct workqueue_struct *cpu_msm8226_wq;
...
void msm8226_cpu_boot_completed(struct work_struct *work)
{
	int ret = 0;
	...
	ret = cpu_down(3);
	if (ret < 0)
		pr_err("cpu4 down err ret=%d", ret);
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	put_cpu();
	tizen_b2_3g_cpu_hotplug_workq = create_workqueue("cpu_hotplug");
	if (!tizen_b2_3g_cpu_hotplug_workq) {
		pr_err("%s cannot create tizen_b2_3g_cpu_hotplug_workq\n", __func__);
		goto err;
	}
	...
	cpu_msm8226_wq = alloc_workqueue("cpu_msm8226_wq", WQ_HIGHPRI, 0);
	if (!cpu_msm8226_wq) {
		printk(KERN_ERR "Failed to create cpufreq_msm8226_wq workqueue\n");
	}

	INIT_DELAYED_WORK(&booting_cpu_boost_work, msm8226_cpu_boot_completed);
	...
	queue_delayed_work(cpu_msm8226_wq
	...
	return cpufreq_register_governor(&cpufreq_gov_tizen_b2_3g);

err :
	destroy_workqueue(tizen_b2_3g_cpu_hotplug_workq);
	return -1;
...
{
	cpufreq_unregister_governor(&cpufreq_gov_tizen_b2_3g);
	destroy_workqueue(tizen_b2_3g_cpu_hotplug_workq);
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_TIZEN_B2_3G
