...
static int cpp_init_hardware(struct cpp_device *cpp_dev)
{
	...
	/* set IS_CPP bandwidth twice of max ISP_VFE bandwidth */
	msm_isp_update_bandwidth(ISP_CPP, 197777700*2, 296666550*2);
#else
...
static int cpp_open_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	if (cpp_dev->cpp_open_cnt == 1) {
		...
		pr_debug("cpp-debug: Attaching cpp iommu during open\n");
...
static int cpp_close_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	if (cpp_dev->cpp_open_cnt == 0) {
		...
		pr_debug("cpp-debug: Detaching cpp iommu after close \n");
...
#ifdef CONFIG_MACH_RINATO_3G_OPEN
			/* set IS_CPP bandwidth twice of max ISP_VFE bandwidth */
			rc = msm_isp_update_bandwidth(ISP_CPP, 197777700*2, 296666550*2);
#else
...
static int __devinit cpp_probe(struct platform_device *pdev)
{
	...
	//attach during init
	iommu_attach_device(cpp_dev->domain, cpp_dev->iommu_ctx);
	pr_debug("cpp-debug: Attaching cpp iommu during probe \n");
	...
	//detach after init
	iommu_detach_device(cpp_dev->domain, cpp_dev->iommu_ctx);
	pr_err("cpp-debug: Detaching cpp iommu after probe \n");
