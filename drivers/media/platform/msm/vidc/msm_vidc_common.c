...
static int msm_vidc_deinit_core(struct msm_vidc_inst *inst)
{
	...
	/*
	 * If Video core is deinited immediately after it is opened,
	 * modify the state to VIDC_CORE_INIT_DONE.
	 * Because if the state is VIDC_CORE_INIT, it does not
	 * go through a certain process when deinit.
	 */
	if (core->state == VIDC_CORE_INIT) {
		core->state = VIDC_CORE_INIT_DONE;
		no_unload_delay = 1;
	...
	int desired_state)
{
	...
	/* fix for the specific case of release right after open device node
	 * like using cat command */
	if (flipped_state == MSM_VIDC_CORE_INIT
			&& desired_state == MSM_VIDC_CORE_UNINIT)
