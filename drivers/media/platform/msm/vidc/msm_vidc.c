...
void *msm_vidc_open(int core_id, int session_type)
{
	...
	core->num_inst++;
	pr_info(VIDC_DBG_TAG "Opening video instance: %p, %d, %s\n",
		VIDC_INFO, inst, core->num_inst,
		session_type == MSM_VIDC_DECODER ? "vdec" : "venc");
...
int msm_vidc_close(void *instance)
{
	...
	msm_smem_delete_client(inst->mem_client);
	pr_info(VIDC_DBG_TAG "Closed video instance: %p, %d\n", VIDC_INFO,
			inst, core->num_inst);
	core->num_inst--;
	kfree(inst);
