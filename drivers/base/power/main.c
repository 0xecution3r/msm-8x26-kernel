...
void dpm_resume(pm_message_t state)
{
	...
	/* Restore mmss_s0_axi clock status */
	if (sirano)
	{
		struct clk *clk;
		clk = clk_get(NULL, "mmss_s0_axi_clk");
		if (IS_ERR(clk)) {
			printk("can not get clk - mmss_s0_axi\n");
		} else {
			if (clk->count == 0) {
				printk("mmss_s0_axi_clk count=[%d]\n", clk->count);
				ret = clk_prepare_enable(clk);
				printk("mmss_s0_axi_clk count=[%d][%d]\n", clk->count, ret);
				} else {
					printk("mmss_s0_axi_clk count=[%d] sirano=0\n", clk->count);
				}
		}
...
int dpm_suspend(pm_message_t state)
{
	...
	/* Store mmss_s0_axi clock status */
	clk = clk_get(NULL, "mmss_s0_axi_clk");
	if (IS_ERR(clk)) {
		printk("can not get clock\n");
	} else {
		if (clk->count != 0) {
			printk("mmss_s0_axi_clk count=[%d]\n", clk->count);
