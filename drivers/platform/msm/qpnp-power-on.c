...
#define QPNP_PON_PBL_STATUS(base)		(base + 0x7)
...
static struct qpnp_pon *gloval_pon;

void gpio_keys_send_fake_powerkey(int value)
{
	struct input_dev *input = gloval_pon->pon_input;
...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	wake_lock_timeout(&pon->key_wake_lock, WAKELOCK_TIME);
...
#ifdef HARD_KEY_BOOSTER
	if ((cfg->key_code == KEY_POWER) && ((pon_rt_sts & pon_rt_bit) != 0))
		schedule_work(&pon->boost_work);
#endif
...
#ifdef CONFIG_SLEEP_MONITOR
	if ((pon->powerkey_state) && pon->press_cnt < 0xffff)
		pon->press_cnt++;
#endif
		...
		struct device_attribute *attr, char *buf)
{
	...
	dev_info(dev, "%s: powerkey_state= %d\n",
		__func__, pon->powerkey_state);

	if (pon->powerkey_state)
		sprintf(buf, "PRESS");
	else
		sprintf(buf, "RELEASE");
...
static int qpnp_pon_get_sleep_monitor_cb(void* priv, unsigned int *raw_val, int check_level, int caller_type);

static struct sleep_monitor_ops  qpnp_pon_sleep_monitor_ops = {
	 .read_cb_func =  qpnp_pon_get_sleep_monitor_cb,
};
...
static int qpnp_pon_get_sleep_monitor_cb(void* priv, unsigned int *raw_val, int check_level, int caller_type)
{
	...
	*raw_val = ((state & STATE_MARK) << STATE_BIT) |\
			(pon->press_cnt & CNT_MARK);

	if (pon->press_cnt > PRETTY_MAX)
		pretty = PRETTY_MAX;
	else
		pretty = pon->press_cnt;

	pon->press_cnt = 0;

	dev_dbg(&pon->spmi->dev, "%s: raw_val[0x%08x], check_level[%d], state[%d], pretty[%d]\n",
...
static int __devinit qpnp_pon_probe(struct spmi_device *spmi)
{
...
#ifdef HARD_KEY_BOOSTER
	INIT_WORK(&pon->boost_work, boost_work_func);
#endif
	INIT_DELAYED_WORK(&pon->bark_work, bark_work_func);

	/* Print PON registers for debug */
	rc = spmi_ext_register_readl(pon->spmi->ctrl, pon->spmi->sid,
			QPNP_PON_PBL_STATUS(pon->base), pon_regs, sizeof(pon_regs));
	for (i=0; i<sizeof(pon_regs); i++)
		dev_info(&pon->spmi->dev,
			"PON_READ 0x%X : %02x\n",
			QPNP_PON_PBL_STATUS(pon->base)+i, pon_regs[i]);
	...
	pon->resume_state = true;
#endif
#ifdef CONFIG_DISPLAY_EARLY_DPMS
	device_set_early_complete(&spmi->dev, EARLY_COMP_SLAVE);
#endif

#ifdef CONFIG_SLEEP_MONITOR
	sleep_monitor_register_ops(pon, &qpnp_pon_sleep_monitor_ops,
			SLEEP_MONITOR_KEY);
#endif
...
#ifdef HARD_KEY_BOOSTER
	cancel_work_sync(&pon->boost_work);
#endif
