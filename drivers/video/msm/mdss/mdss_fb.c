		...
		int check_level, int caller_type)
{
	...
	*raw_val = cnt | state << 24;
	...
	pr_info("%s: caller[%d], dpms[%d] panel on[%d], raw[0x%x]\n",
			__func__, caller_type, mfd->dpms, ret, *raw_val);
...
static int mdss_fb_probe(struct platform_device *pdev)
{
	...
	device_set_early_complete(mfd->fbi->dev, EARLY_COMP_MASTER);
#endif
...
{
	struct mdss_panel_data *pdata;
#ifdef CONFIG_MSM_MDSS_BACKLIGHT
	/* FixMe: need to set proper config */
...
static bool mdss_fb_check_blank(int cur, int next)
{
	bool ret = false;
	...
	pr_info("[%d->%d]%s\n", cur, next, ret ? "" : "[invalid]");

	return ret;
}

#ifdef CONFIG_MSM_KGSL_DRM
static int mdss_fb_send_dpms_event(int index, int blank_mode)
{
	...
	pr_info("[fb%d]blank[%d]dpms[%d]\n", index, blank_mode, dpms);

	event.index = index;
	event.data = (void *)dpms;
	mdss_fb_nb_send_event(MSM_FB_SET_DPMS, (void *)&event);
out:
	return ret;
}
#endif
			     ...
			     int op_enable)
{
	...
	case FB_BLANK_NORMAL:
		...
		mfd->dpms = blank_mode;
#ifdef CONFIG_MSM_KGSL_DRM
		mdss_fb_send_dpms_event(mfd->index, blank_mode);
#endif
		break;
	case FB_BLANK_VSYNC_SUSPEND:
	case FB_BLANK_POWERDOWN:
...
static int mdss_fb_blank(int blank_mode, struct fb_info *info)
{
	...
	if (!mdss_fb_check_blank(mfd->dpms, blank_mode)) {
		ret = -EINVAL;
		goto out;
	}
						...
						unsigned long *argp)
{
	...
	if (disp_commit.wait_for_finish) {
		pr_info("[sync]dpms[%d]\n", mfd->dpms);
		if (mfd->dpms == FB_BLANK_VSYNC_SUSPEND) {
			ret = fb_blank(info, FB_BLANK_NORMAL);
			if (ret) {
				pr_err("%s:failed to set normal\n", __func__);
				return ret;
			}
		}
	}
	...
	if (disp_commit.wait_for_finish) {
		if (mfd->dpms == FB_BLANK_NORMAL) {
			ret = fb_blank(info, FB_BLANK_VSYNC_SUSPEND);
			if (ret)
				pr_err("%s:failed to set vsync_suspend\n", __func__);
		}
		pr_info("[sync]dpms[%d]ret[%d]\n", mfd->dpms, ret);
	}
...
#ifdef CONFIG_DISPLAY_EARLY_DPMS
	device_set_early_complete(&fb_pdev->dev, EARLY_COMP_MASTER);
#endif
