...
static DEFINE_MUTEX(mdp_pm_lock);
...
bool mdss_mdp_check_suspended(struct device *dev, bool suspended, const char *func)
{
	bool ret;
	...
	if (ret != suspended)
		pr_err("ret[%d]dev[%s]cnt[%d][%s]\n",
			ret, (dev->driver && dev->driver->name) ? dev->driver->name : "null",
			atomic_read(&dev->power.usage_count), func);

	return ret;
}

int mdss_mdp_runtime_get_sync(struct device *dev, const char *func)
{
	int ret;
	...
	if (ret < 0)
		pr_info("ret[%d]dev[%s]suspended[%d]cnt[%d][%s]\n",
			ret, dev_name(dev), pm_runtime_suspended(dev),
			atomic_read(&dev->power.usage_count), func);

	return ret;
}

int mdss_mdp_runtime_put_sync(struct device *dev, const char *func)
{
	int ret;
...
int mdss_iommu_ctrl(int enable)
{
	...
	pr_debug("[%s]attached[%d]ref[%d][%pS]in\n",
		enable ? "on" : "off", mdata->iommu_attached, mdata->iommu_ref_cnt,
	...
	pr_debug("[%s]attached[%d]ref[%d][%pS]out[%d]\n",
		enable ? "on" : "off", mdata->iommu_attached, mdata->iommu_ref_cnt,
	...
	if (IS_ERR_VALUE(rc)) {
		pr_info("[%pS]on[%d]ref[%d]ret[%d]\n",
			__builtin_return_address(0), enable, mdata->iommu_ref_cnt, rc);
		return rc;
	}
	else
		...
		if (!enable) {
			if (!mdata->handoff_pending)
				msm_bus_scale_client_update_request(
						mdata->bus_hdl, 0);
			mdss_iommu_ctrl(0);
			mdss_mdp_runtime_put_sync(&mdata->pdev->dev, __func__);
		} else {
			mdss_mdp_runtime_get_sync(&mdata->pdev->dev, __func__);
			msm_bus_scale_client_update_request(
				mdata->bus_hdl, mdata->curr_bw_uc_idx);
                       if (!mdata->handoff_pending) {
				if (IS_ERR_VALUE(mdss_iommu_ctrl(1)))
...
void mdss_mdp_clk_ctrl(int enable, int isr, const char *func)
{
	struct mdss_data_type *mdata = mdss_mdp_get_mdata();
			...
			unsigned long cmd, void *_data)
{
	...
	case KGSL_DRM_SET_CLK: {
		...
		pr_info("clk %d\n", (int)_data);
		mdss_mdp_clk_ctrl(enable, false, __func__);
	}
		break;
	...
	case KGSL_DRM_DPMS_CTRL: {
		struct msm_fb_data_type *mfd;
		struct kgsl_drm_nb_event *event =
			(struct kgsl_drm_nb_event *)_data;
		int idx = event->crtc, blank;
		...
		pr_info("KGSL_DRM_DPMS_CTRL[%d]", (int)event->data);
		if (!lock_fb_info(mfd->fbi)) {
			ret = -NOTIFY_BAD;
			goto out;
		}
		...
		case DRM_MODE_DPMS_OFF:
			...
			break;
		default:
			pr_err("invalid dpms[%d]\n", (int)event->data);
			unlock_fb_info(mfd->fbi);
			ret = NOTIFY_BAD;
			goto out;
		}

		if (fb_blank(mfd->fbi, blank))
			ret = NOTIFY_BAD;
		unlock_fb_info(mfd->fbi);
	}
		break;
...
static int mdss_mdp_cx_ctrl(struct mdss_data_type *mdata, int enable)
{
	...
	if (enable) {
		...
		pr_info("Enabling CX power rail\n");
		...
		pr_info("Disabling CX power rail\n");
...
void mdss_mdp_batfet_ctrl(struct mdss_data_type *mdata, int enable)
{
	...
	if (!mdata->batfet) {
		if (enable) {
			...
			pr_info("Batfet regulator disable w/o enable\n");
			return;
		}
	}
...
{
	if (!mdata->fs) {
		pr_info("failed to get fs\n");
		return;
	}

	pr_debug("[%s]ulps[%d]fs_ena[%d]\n",
		on ? "on" : "off", mdata->ulps, mdata->fs_ena);
	...
	pr_debug("[%s]ulps[%d]fs_ena[%d]done\n",
		on ? "on" : "off", mdata->ulps, mdata->fs_ena);
}
...
int mdss_mdp_footswitch_ctrl_ulps(int on, struct device *dev)
{
	...
	mutex_lock(&mdp_pm_lock);

	if (on && mdata->ulps) {
		pr_debug("[on]ulps[%d]\n", mdata->ulps);
		mdss_mdp_check_suspended(dev, true, __func__);
		mdss_mdp_runtime_get_sync(dev, __func__);
		if (IS_ERR_VALUE(mdss_iommu_ctrl(1)))
			pr_err("IOMMU attach failed\n");
		mdss_hw_init(mdata);
		mdata->ulps = false;
		changed = true;
		pr_debug("[on]ulps[%d]done\n", mdata->ulps);
	} else if (!on && !mdata->ulps) {
		pr_debug("[off]ulps[%d]\n", mdata->ulps);
		mdss_mdp_check_suspended(dev, false, __func__);
		mdata->ulps = true;
		changed = true;
		mdss_mdp_runtime_put_sync(dev, __func__);
		pr_debug("[off]ulps[%d]done\n", mdata->ulps);
	}

	printk("mdp_ulps[%s]pm[%s]%s\n",
		mdata->ulps ? "on" : "off", pm_runtime_suspended(dev) ? "off" : "on",
		changed ? "" : "[same]");

	mutex_unlock(&mdp_pm_lock);
	return 0;
}

int mdss_mdp_fb_runtime_active(bool on)
{
	struct msm_fb_data_type *mfd;
	struct mdss_mdp_ctl *ctl;
	int ret = 0;

	mfd = msm_fb_get_data_type(mdss_fb_get_primary_fb_index());
	if (!mfd) {
		pr_err("failed to get mfd\n");
		ret = -EFAULT;
		goto out;
	}
	...
	ctl = mfd_to_ctl(mfd);
	if (!ctl) {
		pr_info("[%s]bypass:invalid state\n", on ? "on" : "off");
		ret = -EPERM;
		goto out;
	}

	if (!ctl->ulps_trigger) {
		pr_info("[%s]bypass:not support ulps_trigger\n",
			on ? "on" : "off");
		ret = -EPERM;
	} else
		ret = ctl->ulps_trigger(ctl, on);
...
static int mdss_mdp_pm_suspend(struct device *dev)
{
	...
	pr_debug("clk_ena[%d]ulps[%d]fs_ena[%d]\n",
		mdata->clk_ena, mdata->ulps, mdata->fs_ena);

	ret = mdss_mdp_suspend_sub(mdata);

	pr_info("clk_ena[%d]ulps[%d]fs_ena[%d %d]ret[%d]\n",
		mdata->clk_ena, mdata->ulps, mdata->fs_ena,
		mdata->suspend_fs_ena, ret);
...
static int mdss_mdp_runtime_resume(struct device *dev)
{
	...
	pr_debug("clk_ena[%d]ulps[%d]fs_ena[%d]done\n",
		mdata->clk_ena, mdata->ulps, mdata->fs_ena);
