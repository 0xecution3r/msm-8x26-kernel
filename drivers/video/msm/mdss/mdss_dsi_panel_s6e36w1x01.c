...
#ifdef CONFIG_SMART_DIMMING_S6E36W1X01
#include <linux/smart_dimming_s6e36w1x01.h>
#endif
#define DT_CMD_HDR 6
...
static void panel_self_clock_on(bool enable);
static int panel_update_brightness(struct backlight_device *bd, int level);
...
static int mdss_dsi_panel_partial_update(struct mdss_panel_data *pdata)
{
	...
	pr_info("%s: caset[%x][%x][%x][%x]paset[%x][%x][%x][%x]\n", __func__,
...
{
	static const unsigned char gamma_360cd[] = {
...
static int mdss_dsi_panel_read_mtp(struct mdss_panel_data *pdata)
{
	...
	length = mdss_dsi_cmd_receive(ctrl,
				&msd.ldi_code_cmd.cmds[0], LDI_CODE_LEN);
	memcpy(msd.ldi_code, ctrl->rx_buf.data, LDI_CODE_LEN);
	/* GAMMA */
...
void panel_mdnie_set(enum mdnie_scenario scenario)
{
	...
	switch (scenario) {
	case SCENARIO_GRAY:
		mdss_dsi_panel_cmds_send(ctrl_pdata, &msd.gray_tune_cmd);
		break;
	case SCENARIO_NEGATIVE:
		mdss_dsi_panel_cmds_send(ctrl_pdata, &msd.neg_tune_cmd);
		break;
	case SCENARIO_CURTAIN:
		mdss_dsi_panel_cmds_send(ctrl_pdata, &msd.curtain_tune_cmd);
		break;
	case SCENARIO_GRAY_NEGATIVE:
		mdss_dsi_panel_cmds_send(ctrl_pdata, &msd.gray_negative_cmd);
		break;
	default:
...
void panel_mdnie_outdoor_set(enum mdnie_outdoor on)
{
	struct mdss_panel_data *pdata = msd.mpd;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
...
{
	struct mdss_dsi_ctrl_pdata *ctrl = NULL;
	struct mdnie_lite_device *mdnie = msd.mdnie;
	...
	pr_info("%s:power[%d]alpm_on[%d]alpm_mode[%d]\n",
		__func__, msd.power, msd.alpm_on, msd.alpm_mode);
	...
	if (msd.alpm_mode) {
		if (msd.alpm_on) {
			...
			if (msd.self_mode)
				panel_self_clock_on(msd.scm_on);
	...
	if (ctrl->on_cmds.cmd_cnt) {
		if (!msd.boot_power_on) {
			...
			msd.boot_power_on = 0;
			pr_info("%s: panel_on_skip\n", __func__);
		}
	}

	if (!msd.mtp_read_done) {
		mdss_dsi_panel_read_mtp(pdata);
		msd.mtp_read_done = true;
...
out:
	if (mdnie->scenario == SCENARIO_GRAY ||
		mdnie->scenario == SCENARIO_NEGATIVE ||
		mdnie->scenario == SCENARIO_GRAY_NEGATIVE)
		panel_mdnie_set(mdnie->scenario);

	if (mdnie->outdoor == OUTDOOR_ON)
		panel_mdnie_outdoor_set(mdnie->outdoor);

alpm_exit:
	pr_info("%s:power[%d]alpm_on[%d]alpm_mode[%d]done\n",
		__func__, msd.power, msd.alpm_on, msd.alpm_mode);
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	msd.support_mdnie = of_property_read_bool(np,
			"qcom,mdss-dsi-mdnie-lite-enable");
	...
	mdss_dsi_parse_dcs_cmds(np, &msd.ltps_def_cmd,
			"qcom,mdss-dsi-ltps-def-command",
			"qcom,mdss-dsi-ltps-def-command-state");
	mdss_dsi_parse_dcs_cmds(np, &msd.ldi_code_cmd,
			"qcom,mdss-dsi-ldi-manu-code-command",
			"qcom,mdss-dsi-ldi-manu-code-command-state");
	...
	mdss_dsi_parse_dcs_cmds(np, &msd.hbm_off_cmd,
	...
	mdss_dsi_parse_dcs_cmds(np, &msd.scm_on_cmds,
			"qcom,mdss-dsi-scm-on-commands",
			"qcom,mdss-dsi-scm-on-commands-state");
	mdss_dsi_parse_dcs_cmds(np, &msd.scm_off_cmds,
			"qcom,mdss-dsi-scm-off-commands",
			"qcom,mdss-dsi-scm-off-commands-state");
	...
	mdss_dsi_parse_dcs_cmds(np, &msd.gray_tune_cmd,
			"qcom,mdss-dsi-gray-tune-command",
			"qcom,mdss-dsi-gray-tune-command-state");
	mdss_dsi_parse_dcs_cmds(np, &msd.neg_tune_cmd,
			"qcom,mdss-dsi-negative-tune-command",
			"qcom,mdss-dsi-negative-tune-command-state");
	mdss_dsi_parse_dcs_cmds(np, &msd.outdoor_tune_cmd,
			"qcom,mdss-dsi-outdoor-tune-command",
			"qcom,mdss-dsi-outdoor-tune-command-state");
	mdss_dsi_parse_dcs_cmds(np, &msd.curtain_tune_cmd,
			"qcom,mdss-dsi-curtain-tune-command",
			"qcom,mdss-dsi-curtain-tune-command-state");
	mdss_dsi_parse_dcs_cmds(np, &msd.gray_negative_cmd,
			"qcom,mdss-dsi-gray-negative-command",
			"qcom,mdss-dsi-gray-negative-command-state");
	mdss_dsi_parse_dcs_cmds(np, &msd.mcd_on_cmd,
			"qcom,mdss-dsi-mcd-test-on-command",
			"qcom,mdss-dsi-mcd-test-on-command-state");
	mdss_dsi_parse_dcs_cmds(np, &msd.mcd_off_cmd,
			"qcom,mdss-dsi-mcd-test-off-command",
			"qcom,mdss-dsi-mcd-test-off-command-state");
...
static void panel_self_clock_on(bool enable)
{
	...
	if (enable)
		mdss_dsi_panel_cmds_send(ctrl_pdata, &msd.scm_on_cmds);
	else
		mdss_dsi_panel_cmds_send(ctrl_pdata, &msd.scm_off_cmds);

	msd.self_mode = enable;
	pr_info("%s: SELF CLOCK[%s].\n", __func__, enable ? "ON" : "OFF");
...
{
	pr_info("%s[%d][%s]\n", __func__, msd.power, current->comm);
...
static int mdss_dsi_panel_set_power(struct lcd_device *ld, int power)
{
	...
	pr_info("%s[%d->%d][%s]\n", __func__, msd.power, power, current->comm);
	msd.power = power;

	return ret;
}

static int mdss_dsi_panel_early_set_power(struct lcd_device *ld, int power)
{
	int ret = 0;
	...
	pr_info("%s[%d->%d][%s]image_cnt[%d]\n", __func__,
			msd.power, power, current->comm, msd.dbg_cnt);
...
static int mdss_dsi_panel_set_brightness(struct backlight_device *bd)
{
	...
	if (msd.aid_full) {
		mdss_dsi_panel_cmds_send(ctrl_pdata, &msd.ltps_def_cmd);
		pr_info("%s:recover aor(ltps) setting\n", __func__);
		msd.aid_full = false;
	}
...
static struct lcd_ops mdss_dsi_disp_props = {
	...
	.early_set_power = mdss_dsi_panel_early_set_power,
};
				...
				struct device_attribute *attr, char *buf)
{
	...
	sprintf(temp, "%02x%02x%02x%02x%02x\n",
		msd.ldi_code[0], msd.ldi_code[1], msd.ldi_code[2],
		msd.ldi_code[3], msd.ldi_code[4]);
	strcat(buf, temp);
...
static void panel_mcd_test_on(void)
{
	struct mdss_panel_data *pdata = msd.mpd;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	...
	if (msd.mcd_on)
		mdss_dsi_panel_cmds_send(ctrl_pdata, &msd.mcd_on_cmd);
	 else
		mdss_dsi_panel_cmds_send(ctrl_pdata, &msd.mcd_off_cmd);

	msleep(100);

	pr_info("%s: MCD TEST: %s.\n", __func__, msd.mcd_on ? "ON" : "OFF");
}
				...
				size_t size)
{
	...
	if (msd.power > FB_BLANK_UNBLANK) {
		pr_err("mcd test before lcd enable.\n");
		return -EPERM;
	}
...
static void panel_enter_hbm_mode(void)
{
	...
	mdss_dsi_cmds_send(ctrl_pdata, &msd.test_key_on_cmd.cmds[1], 1, 0);
	mdss_dsi_panel_cmds_send(ctrl_pdata, &msd.hbm_cmd);
	mdss_dsi_cmds_send(ctrl_pdata, &msd.update_gamma.cmds[0], 1, 0);
	mdss_dsi_cmds_send(ctrl_pdata, &msd.test_key_off_cmd.cmds[1], 1, 0);
...
static void panel_exit_hbm_mode(void)
{
	...
	mdss_dsi_cmds_send(ctrl_pdata, &msd.test_key_on_cmd.cmds[1], 1, 0);
	mdss_dsi_cmds_send(ctrl_pdata, &msd.hbm_off_cmd.cmds[0], 1, 0);
	mdss_dsi_cmds_send(ctrl_pdata, &msd.update_gamma.cmds[0], 1, 0);
	mdss_dsi_cmds_send(ctrl_pdata, &msd.test_key_off_cmd.cmds[1], 1, 0);
...
static int panel_alpm_handler(int event)
{
	...
	pr_debug("%s:event[%d]alpm_on[%d]alpm_mode[%d]\n",
		__func__, event, msd.alpm_on, msd.alpm_mode);
	...
	pr_info("%s:event[%d]alpm_on[%d]alpm_mode[%d]ret[%d]\n",
		__func__, event, msd.alpm_on, msd.alpm_mode, ret);
...
{
	if (!strncmp(buf, "on", 2))
		msd.ao_mode = AO_NODE_ALPM;
	else if (!strncmp(buf, "off", 3))
		msd.ao_mode = AO_NODE_OFF;
	else if (!strncmp(buf, "self", 4))
		msd.ao_mode = AO_NODE_SELF;
	else
					  ...
					  const char *buf, size_t size)
{
	...
	dev_info(dev, "[mDNIe]cur[%d]new[%d]\n",
					  ...
					  const char *buf, size_t size)
{
	...
	if (value >= OUTDOOR_MAX) {
		pr_warn("invalid outdoor mode set\n");
		return -EINVAL;
	}
	mdnie->outdoor = value;
...
void panel_mdnie_lite_init(void)
{
	...
	bool cmd_cfg_cont_splash)
{
...
#if defined(CONFIG_LCD_CLASS_DEVICE)
	lcd_device = lcd_device_register("s6e36w1x01", &pdev->dev, &msd,
					&mdss_dsi_disp_props);
...
#if defined(CONFIG_BACKLIGHT_CLASS_DEVICE)
	bd = backlight_device_register("s6e36w1x01-bl", &lcd_device->dev,
			&msd, &bd_ops, NULL);
	if (IS_ERR(bd)) {
		rc = PTR_ERR(bd);
	...
	if (msd.support_mdnie)
		panel_mdnie_lite_init();
