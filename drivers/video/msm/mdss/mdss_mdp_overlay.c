				       ...
				       struct mdss_mdp_pipe **ppipe)
{
	...
	if (((mixer->type == MDSS_MDP_MIXER_TYPE_WRITEBACK) &&
		!mdp5_data->mdata->has_wfd_blk) || reset_smp)
				...
				struct mdp_overlay *req)
{
	...
	pr_info("[fb%d]id[0x%x]z[%d]type[%d 0x%x]src[%d %d %d]"	\
		"r[%d %d %d %d -> %d %d %d %d][%d %s]\n",
		mfd->index, req->id, req->z_order, req->pipe_type, req->flags,
				...
				struct mdp_display_commit *data)
{
	...
	list_for_each_entry(pipe, &mdp5_data->pipes_used, used_list) {
		...
		if (pipe->back_buf.num_planes) {
			...
			pr_info("no buf detected pipe[0x%x] use solid fill\n",
					pipe->ndx);
			buf = NULL;
		}
		...
		struct device_attribute *attr, char *buf)
{
	...
	list_for_each_entry_safe(pipe, next, &mdp5_data->pipes_used,
		...
		pos += sprintf(buf+pos, "mixer_stage[%d]",
			pipe->mixer_stage);
					  ...
					  u32 cmd, void __user *argp)
{
	...
	case MSMFB_OVERLAY_SET:
		...
		if (!ret) {
			if (mfd->panel_info->cont_splash_enabled) {
				pr_info("handle cont_splash\n");
				ret = mdss_mdp_overlay_start(mfd);
				if (ret)
					pr_err("fb[%d]failed to start ovly[%d]\n", mfd->index, ret);
				else
					mdss_mdp_overlay_cleanup(mfd);
			}
	...
	case MSMFB_OVERLAY_UNSET:
		if (!IS_ERR_VALUE(copy_from_user(&val, argp, sizeof(val)))) {
			if (mfd->dpms == FB_BLANK_VSYNC_SUSPEND) {
				ret = fb_blank(mfd->fbi, FB_BLANK_NORMAL);
				if (ret) {
			...
			if (mfd->dpms == FB_BLANK_NORMAL) {
				ret = fb_blank(mfd->fbi, FB_BLANK_VSYNC_SUSPEND);
				if (ret)
