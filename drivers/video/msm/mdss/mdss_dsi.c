...
static int mdss_dsi_off(struct mdss_panel_data *pdata)
{
	...
	if (pdata->panel_info.type == MIPI_CMD_PANEL)
		mdss_dsi_clk_ctrl(ctrl_pdata, DSI_ALL_CLKS, 1, __func__);
...
bool mdss_dsi_ulps_feature_enabled(
	...
	int enable)
{
	...
	if (enable && !ctrl_pdata->ulps) {
		...
		ret = mdss_dsi_clk_ctrl(ctrl_pdata, DSI_ALL_CLKS, 1, __func__);
		if (ret) {
		...
		mdss_dsi_clk_ctrl(ctrl_pdata, DSI_LINK_CLKS, 0, __func__);
		...
		mdss_dsi_clk_ctrl(ctrl_pdata, DSI_BUS_CLKS, 0, __func__);
		...
		ret = mdss_dsi_clk_ctrl(ctrl_pdata, DSI_BUS_CLKS, 1, __func__);
		if (ret) {
		...
		ret = mdss_dsi_clk_ctrl(ctrl_pdata, DSI_LINK_CLKS, 1, __func__);
		if (ret) {
...
error:
	if (ret) {
		pr_info("%s:[%s]ulps[%d]lane[0x%x]\n",
			"dsi_ulps", enable ? "on" : "off",
			ctrl_pdata->ulps, lane_status);
		dump_stack();
	}
...
int mdss_dsi_runtime_active(struct mdss_dsi_ctrl_pdata *ctrl,
	bool on, const char *str)
{
	int ret;

	ret = mdss_mdp_fb_runtime_active(on);

	if (ret && on)
		pr_info("%s:on:ret[%d][%s]\n", __func__, ret, str);
...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	mdss_dsi_clk_ctrl(ctrl_pdata, DSI_BUS_CLKS, 1, __func__);
	if (ret) {
		pr_err("%s: failed to enable bus clocks. rc=%d\n", __func__,
			ret);
		ret = mdss_dsi_panel_power_on(pdata, 0);
...
#ifndef CONFIG_FB_MSM_MDSS_SDC_S6E36W0X01_PANEL
	if (pdata->panel_info.type == MIPI_CMD_PANEL) {
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_UNBLANK:
		pr_info("MDSS_EVENT_UNBLANK\n");
		rc = mdss_dsi_on(pdata);
		mdss_dsi_op_mode_config(pdata->panel_info.mipi.mode,
							pdata);
		if (ctrl_pdata->on_cmds.link_state == DSI_LP_MODE)
			rc = mdss_dsi_unblank(pdata);
		pr_info("MDSS_EVENT_UNBLANK:done[%d]\n", rc);
		break;
	case MDSS_EVENT_PANEL_ON:
		pr_info("MDSS_EVENT_PANEL_ON\n");
		ctrl_pdata->ctrl_state |= CTRL_STATE_MDP_ACTIVE;
		if (ctrl_pdata->on_cmds.link_state == DSI_HS_MODE)
			rc = mdss_dsi_unblank(pdata);
		pr_info("MDSS_EVENT_PANEL_ON:done[%d]\n", rc);
		break;
	case MDSS_EVENT_BLANK:
		pr_info("MDSS_EVENT_BLANK\n");
		if (ctrl_pdata->off_cmds.link_state == DSI_HS_MODE)
			rc = mdss_dsi_blank(pdata);
		pr_info("MDSS_EVENT_BLANK:done[%d]\n", rc);
		break;
	case MDSS_EVENT_PANEL_OFF:
		pr_info("MDSS_EVENT_PANEL_OFF\n");
		ctrl_pdata->ctrl_state &= ~CTRL_STATE_MDP_ACTIVE;
		if (ctrl_pdata->off_cmds.link_state == DSI_LP_MODE)
			rc = mdss_dsi_blank(pdata);
		rc = mdss_dsi_off(pdata);
		pr_info("MDSS_EVENT_PANEL_OFF:done[%d]\n", rc);
		break;
	case MDSS_EVENT_CONT_SPLASH_FINISH:
		pr_info("MDSS_EVENT_CONT_SPLASH_FINISH\n");
		if (ctrl_pdata->off_cmds.link_state == DSI_LP_MODE)
			rc = mdss_dsi_blank(pdata);
		ctrl_pdata->ctrl_state &= ~CTRL_STATE_MDP_ACTIVE;
		rc = mdss_dsi_cont_splash_on(pdata);
		pr_info("MDSS_EVENT_CONT_SPLASH_FINISH:done[%d]\n", rc);
		break;
	case MDSS_EVENT_PANEL_CLK_CTRL:
		pr_debug("MDSS_EVENT_PANEL_CLK_CTRL[%d]\n", (int)arg);
		mdss_dsi_clk_req(ctrl_pdata, (int)arg);
		pr_debug("MDSS_EVENT_PANEL_CLK_CTRL:done[%d]\n", rc);
		break;
	case MDSS_EVENT_DSI_CMDLIST_KOFF:
		pr_debug("MDSS_EVENT_DSI_CMDLIST_KOFF\n");
		mdss_dsi_cmdlist_commit(ctrl_pdata, 1);
		pr_debug("MDSS_EVENT_DSI_CMDLIST_KOFF:done[%d]\n", rc);
		break;
	case MDSS_EVENT_PANEL_UPDATE_FPS:
		pr_info("MDSS_EVENT_PANEL_UPDATE_FPS[%d]\n", (int)arg);
		if (arg != NULL) {
			rc = mdss_dsi_dfps_config(pdata, (int)arg);
			pr_debug("%s:update fps to = %d\n",
				__func__, (int)arg);
		}
		pr_info("MDSS_EVENT_PANEL_UPDATE_FPS:done[%d]\n", rc);
		break;
	case MDSS_EVENT_CONT_SPLASH_BEGIN:
		pr_info("MDSS_EVENT_CONT_SPLASH_BEGIN\n");
		if (ctrl_pdata->off_cmds.link_state == DSI_HS_MODE) {
			/* Panel is Enabled in Bootloader */
			rc = mdss_dsi_blank(pdata);
		} else
			rc = -EACCES;
		pr_info("MDSS_EVENT_CONT_SPLASH_BEGIN:done[%d]\n", rc);
		break;
	case MDSS_EVENT_ENABLE_PARTIAL_UPDATE:
		pr_info("MDSS_EVENT_ENABLE_PARTIAL_UPDATE\n");
		rc = mdss_dsi_ctl_partial_update(pdata);
		pr_info("MDSS_EVENT_ENABLE_PARTIAL_UPDATE:done[%d]\n", rc);
		break;
	case MDSS_EVENT_DSI_ULPS_CTRL:
		pr_debug("MDSS_EVENT_DSI_ULPS_CTRL[%d]\n", (int)arg);
		if (mdss_dsi_ulps_feature_enabled(pdata))
			rc = mdss_dsi_ulps_config(ctrl_pdata, (int)arg);
		pr_debug("MDSS_EVENT_DSI_ULPS_CTRL:done[%d]\n", rc);
		break;
	case MDSS_EVENT_REGISTER_RECOVERY_HANDLER:
		pr_info("MDSS_EVENT_REGISTER_RECOVERY_HANDLER\n");
		rc = mdss_dsi_register_recovery_handler(ctrl_pdata,
			(struct mdss_panel_recovery *)arg);
		pr_info("MDSS_EVENT_REGISTER_RECOVERY_HANDLER:done[%d]\n", rc);
		break;
	default:
