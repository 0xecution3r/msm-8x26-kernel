...
void mdss_dsi_clk_req(struct mdss_dsi_ctrl_pdata *ctrl, int enable)
{
	...
	mdss_dsi_clk_ctrl(ctrl, DSI_ALL_CLKS, enable, __func__);
}
...
void mdss_dsi_pll_relock(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	/* disable dsi clk */
	for (i = 0; i < cnt; i++)
		mdss_dsi_clk_ctrl(ctrl, DSI_LINK_CLKS, 0, __func__);

	/* enable dsi clk */
	for (i = 0; i < cnt; i++)
		mdss_dsi_clk_ctrl(ctrl, DSI_LINK_CLKS, 1, __func__);
}
...
#ifdef CONFIG_MIPI_REG_DEBUG
void mdss_dsi_reg_dump(struct mdss_dsi_ctrl_pdata *ctrl)
{
	u32 i;
	unsigned char *addr = ctrl->ctrl_base;
	pr_info("======= DSI register dump START ========\n");
	for (i = 0; i < 0x200; i++) {
		pr_info("0x%08x: 0x%08x 0x%08x 0x%08x 0x%08x\n",
				(int)(addr + i),
				(u32)MIPI_INP(addr + i),
				(u32)MIPI_INP(addr + i + 0x4),
				(u32)MIPI_INP(addr + i + 0x8),
				(u32)MIPI_INP(addr + i + 0xc));
		i += 0xf;
	}
	pr_info("======= DSI register dump END ========\n");
}
#endif
/*
...
int mdss_dsi_cmdlist_commit(struct mdss_dsi_ctrl_pdata *ctrl, int from_mdp)
{
	...
	if (ctrl->ulps) {
		pr_err("%s:need to exit ulps:from_mdp[%d]flags[0x%x]\n",
			__func__, from_mdp, req->flags);
		dump_stack();
		mutex_unlock(&ctrl->cmd_mutex);
		return -EINVAL;
	}
	...
	mdss_dsi_clk_ctrl(ctrl, DSI_ALL_CLKS, 1, __func__);
	...
	mdss_iommu_ctrl(0);
	mdss_dsi_clk_ctrl(ctrl, DSI_ALL_CLKS, 0, __func__);
