...
struct mdss_mdp_mixer *mdss_mdp_wb_mixer_alloc(int rotator)
{
	...
	if (rotator && mixer->mirror && mixer->mirror->ref_cnt
		&& mixer->mirror->rotator_mode) {
		pr_err("rotator on this mixer in using\n");
		goto error;
	}
		...
		struct mdp_display_commit *data)
{
	...
	pr_debug("roi[%d %d %d %d]\n",
		data->roi.x, data->roi.y, data->roi.w, data->roi.h);

	temp_roi.x = round_down(data->roi.x, ROI_ALIGN);
	temp_roi.y = data->roi.y;
	temp_roi.w = MIN(ALIGN(data->roi.w + (data->roi.x - temp_roi.x),
					ROI_ALIGN), ctl->mixer_left->width);
	...
	*/
	if (!temp_roi.w || !temp_roi.h || ctl->mixer_right ||
		temp_roi.x + temp_roi.w > ctl->mixer_left->width ||
		temp_roi.y + temp_roi.h > ctl->mixer_left->height ||
	 ...
	 u32 type, u32 len)
{
	...
	*/
	if ((type == MDSS_MDP_MIXER_TYPE_WRITEBACK) && !mdata->has_wfd_blk) {
		head[len] = head[len - 1];
		head[len].mirror= &head[len - 1];
		head[len - 1].mirror= &head[len];
	}
...
int mdss_mdp_mixer_pipe_unstage(struct mdss_mdp_pipe *pipe)
{
	...
	pr_debug("ndx=0x%x pnum=%d stage=%d mixer=%d\n", pipe->ndx,
		pipe->num, pipe->mixer_stage, mixer->num);
