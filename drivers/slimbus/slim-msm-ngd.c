...
#ifdef ENABLE_SLIMBUS_DEBUG
static int slimbus_debug_count;
#endif
static int ngd_xfer_msg(struct slim_controller *ctrl, struct slim_msg_txn *txn)
{
	...
	if (!report_sat) {
		if (dev->state == MSM_CTRL_DOWN) {
...
#ifdef ENABLE_SLIMBUS_DEBUG
			slimbus_debug_count++;
			if (slimbus_debug_count > 50)
				panic("ADSP slimbus ctrl down");
#endif
			/*
			...
			/* Clear debug count to avoid false alarm  */
			slimbus_debug_count = 0;
		}
#endif
