				...
				char *dataframe, int length)
{
	...
	if (dataframe[0] == 0x01 && dataframe[1] == 0x01 && dataframe[2] == 0x36 && dataframe[3] != hub_data->ssp_data->strap_status) {
		hub_data->ssp_data->strap_status = dataframe[3];
		schedule_work(&hub_data->ssp_data->payment_work);
	}
#endif
#ifdef CONFIG_SENSORHUB_STAT
