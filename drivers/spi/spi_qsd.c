...
#define SPI_PM_AUTOSUSPEND_DELAY	100
...
static inline irqreturn_t msm_spi_qup_irq(int irq, void *dev_id)
{
	...
	if (dd->suspended && pm_runtime_suspended(dd->dev)) {
...
int ese_spi_reset(struct spi_device *spidev)
{
	int result = 0;
	struct msm_spi *dd;
	...
	result = msm_spi_set_state(dd, SPI_OP_STATE_RESET);
	if (result) {
		pr_err("%s: msm_spi_set_state fail: %d\n", __func__, result);
		return result;
	}

	return result;
}
EXPORT_SYMBOL_GPL(ese_spi_reset);
#endif
...
skip_dma_resources:
	...
	pm_runtime_set_autosuspend_delay(&pdev->dev, SPI_PM_AUTOSUSPEND_DELAY);
