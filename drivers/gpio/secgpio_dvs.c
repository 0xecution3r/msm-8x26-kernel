...
extern unsigned sec_msm_gpiomux_ngpio;
...
static ssize_t secgpio_checked_sleepgpio_read(
	struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t secgpio_checked_gpio_status_read(
	struct device *dev, struct device_attribute *attr, char *buf);
...
static DEVICE_ATTR(gpio_status,0644,
	secgpio_checked_gpio_status_read, NULL);
	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < gdvs->count; i++) {
		chip = gpio_to_chip(i) ;
		value = chip->get(chip, i - chip->base);
		memset(temp_buf, 0, sizeof(char)*20);
		snprintf(temp_buf, 20, "GI[%d] - %d\n ",
			i, value);
		strlcat(buf, temp_buf, PAGE_SIZE);
	}
	return strlen(buf);
