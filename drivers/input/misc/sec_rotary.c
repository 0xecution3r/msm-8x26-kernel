...
static int rotary_ctrl_power(struct rotary_ddata *rotary, bool power)
{
	...
	if (!regulator_pwr) {
		regulator_pwr = regulator_get(&rotary->client->dev, "ofm_vdd");
		if (IS_ERR(regulator_pwr)) {
			dev_err(&rotary->mouse_dev->dev, "%s: failed to get ldo14(ofm_vdd) regulator\n", __func__);
...
static void rotary_motion_handler(struct rotary_ddata *rotary)
{
	...
	atomic_set(&rotary->ofm_pending, 1);
	...
	atomic_set(&rotary->ofm_pending, 0);
...
static irqreturn_t hall_status_detect(int irq, void *dev_id)
{
	...
	if (gpio_get_value(rotary->pdata->motion_pin) &&\
	     !atomic_read(&rotary->ofm_pending)) {
		cancel_delayed_work(&rotary->motion_dwork);
		rotary_motion_handler(rotary);
		schedule_delayed_work(&rotary->motion_dwork,
			msecs_to_jiffies(MOTION_READ_PERIOD));
	}
...
out:
	mutex_unlock(&rotary->hall_mutex);
...
static struct rotary_platform_data *rotary_parse_dt(struct device *dev)
{
	...
	ret = of_property_read_u32(np, "ofm_standby", &pdata->standby_pin);
	if (ret) {
		pdata->standby_pin = -1;
		dev_err(dev, "failed to get ofm_standby property\n");
	} else
		dev_dbg(dev, "%s: pdata->standby_pin: %d.\n",
			__func__, pdata->standby_pin);

	ret = of_property_read_u32(np, "ofm_pwrdown", &pdata->powerdown_pin);
	if (ret) {
		pdata->powerdown_pin = -1;
		dev_err(dev, "failed to get ofm_pwrdown property\n");
	} else
		dev_dbg(dev, "%s: pdata->ofm_pwrdown: %d.\n",
			__func__, pdata->powerdown_pin);

	ret = of_property_read_u32(np, "ofm_motion", &pdata->motion_pin);
	if (ret) {
		pdata->motion_pin = -1;
		dev_err(dev, "failed to get ofm_motion property\n");
	} else
		dev_dbg(dev, "%s: pdata->ofm_motion: %d.\n",
			__func__, pdata->motion_pin);

	pdata->always_on = of_property_read_bool(np, "ofm_always_on");
	dev_dbg(dev, "%s: pdata->always_on: %d.\n",
		__func__, pdata->always_on);

	ret = of_property_read_u32(np, "hall_gpio_a", &pdata->hall_a_pin);
	if (ret) {
		pdata->hall_a_pin = -1;
		dev_err(dev, "failed to get hall_gpio_a property\n");
	} else
		dev_dbg(dev, "%s: pdata->hall_a_pin: %d.\n",
			__func__, pdata->hall_a_pin);

	ret = of_property_read_u32(np, "hall_gpio_b", &pdata->hall_b_pin);
	if (ret) {
		pdata->hall_b_pin= -1;
		dev_err(dev, "failed to get hall_gpio_b property\n");
	} else
		dev_dbg(dev, "%s: pdata->hall_b_pin: %d.\n",
...
static int rotary_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!rotary->pdata) {
		result = -ENXIO;
		dev_err(&client->dev, "%s: Failed to get device tree data.\n", __func__);
		goto err_get_dt;
	}
#else
