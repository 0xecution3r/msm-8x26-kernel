...
static int hall_sensor_power_on(struct hall_sensor_driverdata *ddata)
{
	...
	if (pdata->ldo_name) {
		...
		dev_err(&input_dev->dev, "%s: %s power on", __func__, pdata->ldo_name);
	}
...
static int hall_sensor_power_off(struct hall_sensor_driverdata *ddata)
{
	...
	if (pdata->ldo_name) {
		...
		dev_err(&input_dev->dev, "%s: %s power off", __func__, pdata->ldo_name);
	}
			...
			struct hall_sensor_platform_data *pdata)
{
	...
	/* gpio info */
	ret = of_property_read_u32(np, "hall_sensor,gpio_a", &pdata->gpio_a);
	if (ret) {
		pdata->gpio_a = -1;
	...
	ret = of_property_read_u32(np, "hall_sensor,gpio_b", &pdata->gpio_b);
	if (ret) {
		pdata->gpio_b = -1;
	...
	ret = of_property_read_u32(np, "hall_sensor,gpio_c", &pdata->gpio_c);
	if (ret) {
		pdata->gpio_c = -1;
