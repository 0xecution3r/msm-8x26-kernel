...
static int ofm_ctrl_power(struct ofm *ofm, bool power)
{
	...
	if (!regulator_pwr) {
		regulator_pwr = regulator_get(&ofm->client->dev, "ofm_vdd");
...
static int ofm_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ofm->motion_irq = gpio_to_irq(ofm->pdata->motion_pin);
	if (ofm->motion_irq < 0) {
		dev_err(&ofm->input_dev->dev, "%s: Failed to gpio_to_irq.[%d]\n",
			__func__, ofm->pdata->motion_pin);
		goto err_input_reg;
	}

	result = request_threaded_irq(ofm->motion_irq,
	...
	if (result < 0) {
		dev_err(&ofm->input_dev->dev, "%s: Failed to register interrupt [%d][%d]\n",
			__func__, ofm->motion_irq, result);
		goto err_input_reg;
	}
