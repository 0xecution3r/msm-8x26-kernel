...
{
	struct cyttsp5_mt_data *info = container_of(work, struct cyttsp5_mt_data,
					work_dvfs_chg.work);
...
static void set_dvfs_lock(struct cyttsp5_mt_data *info, uint32_t mode)
{
	...
	struct cyttsp5_touch *touch, u8 *xy_data)
{
	...
	u32 ret = 0;
	/*
	 * call qcom_smc_gearpay for every touch event.
...
#endif
#ifdef TSP_BOOSTER
					set_dvfs_lock(md, TOUCH_BOOSTER_DELAY_OFF);
#endif
...
cyttsp5_get_mt_touches_pr_tch:
		...
		if (tch->abs[CY_TCH_E] == CY_EV_TOUCHDOWN) {
#ifdef TSP_BOOSTER
			set_dvfs_lock(md, TOUCH_BOOSTER_ON);
#endif
