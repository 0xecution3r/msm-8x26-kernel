...
static void sec_update_phy_init(void)
{

	kparam_block_sysfs_write(override_phy_init);

#if defined(CONFIC_MACH_B3_ATT) || defined(CONFIG_MACH_B3_TMO) \
	|| defined(CONFIG_MACH_B3_VMC)
#else
	/* only CONFIG_MACH_B3 case, EUR */
	if (system_rev > 0x8)
		override_phy_init =
			"0x44,0x80,0x78,0x81,0x24,0x82,0x13,0x83,0xffffffff";
#endif

	kparam_unblock_sysfs_write(override_phy_init);

	if (override_phy_init)
		pr_info("[%s] USB_PHYTUNE by override param\n", __func__);
}
#else
static inline void sec_update_phy_init(void)
{
}
#endif
...
static void ulpi_init(struct msm_otg *motg)
{
	...
	while (seq[0] >= 0) {
		if (override_phy_init)
			pr_info("ulpi_ov: write 0x%02x to 0x%02x\n",
					seq[0], seq[1]);
		else
			dev_info(motg->phy.dev, "ulpi_dt: write 0x%02x to 0x%02x\n",
				seq[0], seq[1]);
...
static void msm_otg_init_sm(struct msm_otg *motg)
{
	...
	case USB_OTG:
		if (pdata->otg_control == OTG_USER_CONTROL) {
			 ...
			 */
			ret = wait_for_completion_timeout(&pmic_vbus_init, HZ * 5);
			if (!ret) {
				dev_err(motg->phy.dev, "%s: completion timeout\n", __func__);
