...
static inline struct slp_multi_dev *func_to_smdev
					(struct slp_multi_usb_function *f)
{
	struct device *smdev_pdev;

	smdev_pdev = f->dev->parent;

	return container_of(smdev_pdev, struct slp_multi_dev, dev);
}

struct ffs_func_conf {
	struct ffs_data *ffs_data;
	unsigned long ffs_registered;
	const char *dev_name;
	struct slp_multi_usb_function *f;
};
static struct ffs_func_conf *ffs_config;
...
static int functionfs_ready_callback(struct ffs_data *ffs)
{
	...
	f = ffs_config->f;
	smdev =	func_to_smdev(f);

	if (WARN_ON(test_and_set_bit(0,
			&ffs_config->ffs_registered))) {
		dev_err(f->dev, "Someone already registered!!\n");
		return -EBUSY;
	}

	ffs_config->ffs_data = ffs;

	ret = functionfs_bind(ffs, smdev->cdev);
	if (unlikely(ret < 0)) {
		ffs_config->ffs_data = NULL;
		dev_err(f->dev, "Failed to functionfs_bind(%d)\n", ret);
	} else
		dev_info(f->dev, "Upper-layer is ready to be done\n");
...
static void functionfs_closed_callback(struct ffs_data *ffs)
{
	...
	if (test_and_clear_bit(0,
			&ffs_config->ffs_registered)) {
		if (ffs_config->ffs_data) {
			functionfs_unbind(ffs_config->ffs_data);
			ffs_config->ffs_data = NULL;

			dev_info(ffs_config->f->dev,
				"functinofs_unbind by upper-layer\n");
		}
	} else {
		WARN(1, "Someone remaining??\n");
	}
...
static int functionfs_check_dev_callback(const char *dev_name)
{
	...
	/* Check really using ffs or not on first config */
	list_for_each_entry(f, &smdev->funcs_fconf, fconf_list) {
		if (!strncmp(f->name, dev_name, 3))
			find_ffs = 1;
	}

	if (!find_ffs) {
		dev_err(f->dev, "Not yet added ffs into functions\n");
		return -ENODEV;
	}

	ffs_config->dev_name = dev_name;
	return 0;
}


static int ffs_function_init(struct slp_multi_usb_function *f,
			     struct usb_composite_dev *cdev)
{
	ffs_config = kzalloc(sizeof(struct ffs_func_conf), GFP_KERNEL);
	if (!ffs_config)
		return -ENOMEM;
...
static void ffs_function_cleanup(struct slp_multi_usb_function *f)
{
	functionfs_cleanup();
	kfree(ffs_config);
	ffs_config = NULL;
...
static int ffs_function_bind_config(struct slp_multi_usb_function *f,
				    struct usb_configuration *c)
{
	...
	if (WARN_ON(!ffs_config->ffs_data))
		return -ENODEV;

	if (!test_bit(0, &ffs_config->ffs_registered)) {
		dev_err(f->dev, "Can't going, upper-layer does not ready\n");
		return -ENODEV;
	}

	ret = functionfs_bind_config(c->cdev, c, ffs_config->ffs_data);
	if (unlikely(ret < 0))
		dev_err(f->dev, "Failed to ffs bind_config(%d)\n", ret);

	return ret;
}

static void ffs_function_unbind_config(struct slp_multi_usb_function *f,
				    struct usb_configuration *c)
{
	if (!ffs_config)
		return;
	...
	if (ffs_config->ffs_data) {
		...
		dev_info(f->dev, "functionfs_unbind by lower-layer(maybe)\n");
	}
}

static struct slp_multi_usb_function ffs_function = {
	.name = "ffs",
	...
	if (!rmnet_initialized) {
		...
		while (b) {
			...
			if (ctrl_name && data_name) {
				err = frmnet_init_port(ctrl_name, data_name, NULL);
				if (err) {
...
static int __devinit slp_multi_probe(struct platform_device *pdev)
{
	...
	if (np) {
		...
		if (of_find_property(np, "slp,slp_multi-nluns", NULL))
			of_property_read_u32(np, "slp,slp_multi-nluns",
				&pdata->nluns);
