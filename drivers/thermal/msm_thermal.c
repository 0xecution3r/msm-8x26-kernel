...
static long g_temp = 0;
static void show_all_cpu_temp(void)
{
	enum {
		TSENS_ID_CORE_0_1 = 5,
		TSENS_ID_CORE_2_3 = 2,
	...
	struct tsens_device tsens_devs[TSENS_ID_MAX];
	unsigned long temp[TSENS_ID_MAX] = {0,};
	char buf[256] = {0};
	int sz = 0, ret, i;

	tsens_devs[0].sensor_num = TSENS_ID_CORE_0_1;
	tsens_devs[1].sensor_num = TSENS_ID_CORE_2_3;

	for (i = 0; i < ARRAY_SIZE(tsens_devs); i++) {
		ret = tsens_get_temp(&tsens_devs[i], &temp[i]);
		if (ret < 0) {
			pr_err("%s: tsens %d not initialized\n",
					KBUILD_MODNAME, tsens_devs[i].sensor_num);
			return;
		}
		sz += sprintf(buf + sz, "%lu ", temp[i]);
	}

	pr_info("%s: t: %s\n", KBUILD_MODNAME, buf);
}

static void print_temp_log(struct work_struct *work)
{
	pr_info("%s: t: %lu\n", KBUILD_MODNAME, g_temp);

	if (enabled)
		schedule_delayed_work(&temp_log_work, msecs_to_jiffies(3000));
}
		...
		unsigned long action, void *hcpu)
{
	...
	if (action == CPU_UP_PREPARE || action == CPU_UP_PREPARE_FROZEN) {
		if (core_control_enabled &&
			...
			pr_info("%s: Preventing cpu%d from coming online.\n",
				KBUILD_MODNAME, cpu);
			show_all_cpu_temp();
			return NOTIFY_BAD;
		}
...
static int __ref set_enabled(const char *val, const struct kernel_param *kp)
{
	...
	pr_info("%s: enabled = %d by %s(%d)\n",
			KBUILD_MODNAME, enabled, current->comm, current->pid);
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("%s: attempt to core control:0x%x by %s(%d)\n",
		KBUILD_MODNAME, val, current->comm, current->pid);
	show_all_cpu_temp();
...
int __devinit msm_thermal_init(struct msm_thermal_data *pdata)
{
	...
	INIT_DELAYED_WORK(&temp_log_work, print_temp_log);
	schedule_delayed_work(&temp_log_work, msecs_to_jiffies(500));
