			...
			int termination_current)
{
	...
	/* [3:0]=0000 -> 7.5mA */
	if (termination_current > 170)
		termination_current = 160;
			...
			int fast_charging_current)
{
	...
	/* Set the Fast Charging Current low bit so that the charging
	 * current is defined by High Bits. This is applicable only if
	 * the fast charge current is above 100mA
	 */
	...
	if (charger->cable_type == POWER_SUPPLY_TYPE_BATTERY) {
		...
		if (charger->pdata->chg_float_voltage > charger->voltage_max) {
			data |= max14577_get_float_voltage_data(
				charger->voltage_max);
			dev_dbg(&charger->client->dev, "%s : float voltage in voltage_max (%dmV)\n",
		...
		if (batt_booting_chk) {
			charging_current = charger->charging_current -
							(100 - charger->siop_level);
			pr_info("%s: charging_current[%d], siop[%d]\n",
				__func__, charging_current, charger->siop_level);
			data = 0;
		...
		struct sec_charger_info *charger, int charging_current)
{
	...
	if (charging_current == charger->charging_current)
		charging_current = charger->charging_current -
					(100 - charger->siop_level);
	pr_info("%s: charging_current[%d], siop[%d]\n",
		__func__, charging_current, charger->siop_level);
