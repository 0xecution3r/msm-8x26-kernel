...
static int bcm5935x_read_fod_data(struct bcm5935x_wpc_info *wpc)
{
	...
	pr_info("Vrect = %u, Vbuck = %u ", val_vrect, val_vbuck);
	pr_info("Irect = %u, Ibuck = %u ", val_irect, val_ibuck);
	pr_info("Power = %u, Tdie = %u\n", val_power, val_tdie);
	pr_info("wait_time_for_sample = %u ms ", wait_time_for_sample);
	pr_info("max_power_samples = %u\n", max_power_samples);
