...
/* Note - Android Alarm Implementation is being used though we need to switch
 * to Standard Alarm Timer implementation
 */
					...
					struct sec_battery_info *battery)
{
	...
	dev_info(battery->dev, "%s: full_check_type : (%d)\n",
...
/* #else */
	...
	dev_info(battery->dev,
			"%s: batt_booting_chk:%d, done:%d\n", __func__,
			batt_booting_chk, done);

	if (batt_booting_chk) {
		if (!done) {
			/* if settimeofday is done late. charging time is jumping over ABS time.*/
			/* reset the charging start time to new settimeofday time */
			battery->charging_start_time = ts.tv_sec;
			done = 1;
		}
...
continue_monitor:
	dev_info(battery->dev,
		"%s: S(%s), mode(%s), H(%s), Cable(%d), siop(%d), Event(0x%08x)\n",
		__func__,
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case BATT_VFOCV:
		...
		break;
	case BATT_VOL_ADC:
		battery->voltage_adc = 0;
		battery->voltage_adc = sec_bat_vol_adc(battery);
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			battery->voltage_adc);
		break;
		...
		struct sec_battery_info *battery)
{
	...
	pdata->polling_time = kzalloc(sizeof(*pdata->polling_time) * len,
			GFP_KERNEL);
...
/* #else */
	...
	wake_lock_timeout(&battery->booting_check_lock, HZ * 60);
