...
/* Currently some parameters populated in DT for battery
 * are not correct due to which charging functionality
 * is affected. Hence, the Device Tree approach would be
 * taken once the correct battery parameters are available
*/

#if 0
#if CONFIG_OF
static int max14577_parse_dt(struct sec_charger_info *charger)
{
	struct device_node *np = of_find_node_by_name(NULL, "sec-charger");
	...
	if (np == NULL) {
		...
		ret = of_property_read_u32(np, "sec-charger,chg_float_voltage",
					&pdata->chg_float_voltage);
		ret = of_property_read_u32(np, "sec-charger,ovp_uvlo_check_type",
					&pdata->ovp_uvlo_check_type);
		ret = of_property_read_u32(np, "sec-charger,full_check_type",
					&pdata->full_check_type);

		p = of_get_property(np, "sec-charger,input_current_limit", &len);
		len = len / sizeof(u32);
		...
		for(i = 0; i < len; i++) {
			pdata->charging_current[i].input_current_limit = \
				be32_to_cpup(((__be32 *)p) + i);
		}
		p = of_get_property(np, "sec-charger,fast_charging_current", &len);
		len = len / sizeof(u32);
		for(i = 0; i < len; i++) {
			pdata->charging_current[i].fast_charging_current = \
				be32_to_cpup(((__be32 *)p) + i);
		}
		p = of_get_property(np, "sec-charger,full_check_current_1st", &len);
		len = len / sizeof(u32);
		for(i = 0; i < len; i++)
			pdata->charging_current[i].full_check_current_1st = \
				be32_to_cpup(((__be32 *)p) + i);

		p = of_get_property(np, "sec-charger,full_check_current_2nd", &len);
		len = len / sizeof(u32);
		for(i = 0; i < len; i++)
...
#else
static int max14577_parse_dt(struct max14577_charger_data *charger)
{
	return 0;
}
#endif
...
static int __devinit sec_charger_probe(struct platform_device *pdev)
{
	...
	/* Charger I2C Address - 4A >> 1 = 0x25 */
	charger->client = max14577->i2c;
	charger->pdata = mfd_pdata->charger_data;

	/*Device Tree Support */
#if 0
	pdata = kzalloc(sizeof(sec_battery_platform_data_t), GFP_KERNEL);
	if (max14577->i2c->dev.of_node) {
		if (!pdata)
			goto err_free1;
		charger->pdata = pdata;
		if (max14577_parse_dt(charger))
			dev_err(&pdev->dev,
