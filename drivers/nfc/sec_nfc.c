...
static irqreturn_t sec_nfc_irq_thread_fn(int irq, void *dev_id)
{
	...
	//printk("[NFC] nfc Read Interrupt is occurred!\n");
#if 1
				...
				size_t count, loff_t *ppos)
{
	...
	if (ret == -EREMOTEIO) {
		dev_err(info->dev, "1send failed: return: %d count: %d\n",
		ret, count);
		ret = -ERESTART;
		goto out;
	}
	...
	if (ret != count) {
		dev_err(info->dev, "2send failed: return: %d count: %d\n",
		ret, count);
		ret = -EREMOTEIO;
	}
...
void sec_nfc_clk_enable(struct device *dev,
							struct sec_nfc_platform_data *pdata)
{

	pr_info("nfc checkt clk!![%p]\n" , pdata->nfc_clk);

	if (pdata->nfc_clk)
	{
		pr_info("nfc Already Enable the CLK");
		return;
	}

	pdata->nfc_clk = clk_get(dev, "nfc_clk");

	pr_info("2 nfc checkt clk!![%p]\n" , pdata->nfc_clk);
	...
	pr_info("Success get the nfc clock from pmic!!\n");
}

void sec_nfc_clk_disable(struct sec_nfc_platform_data *pdata)
{
	if (pdata->nfc_clk) {
		pr_info(" sec_nfc_clk_disable called\n");
		clk_disable_unprepare(pdata->nfc_clk);
		pr_info("3 nfc checkt clk in disable!![%p]\n" , pdata->nfc_clk);
		pdata->nfc_clk = NULL;
		pr_info("4 nfc checkt clk in disable!![%p]\n" , pdata->nfc_clk);
	}
	else
		pr_info(" sec_nfc_clk_disable Not Call!!!!\n");
}
					...
					enum sec_nfc_mode mode)
{
	...
	if (mode == SEC_NFC_MODE_BOOTLOADER)
		if (pdata->firm)
		{
			...
			pr_info("nfc firm value bootloader mode : %d\n", value);
		}
	...
	if (mode != SEC_NFC_MODE_OFF) {
		...
		sec_nfc_clk_enable(info->dev, pdata);
	...
	if(pdata->pvdd !=  -EINVAL)
	{
		value = gpio_get_value(pdata->pvdd);
		pr_info("nfc pvdd value : %d\n", value);
	}
...
static int __sec_nfc_probe(struct device *dev)
{
	...
	pr_info("[NFC]sec nfc check pvdd [%d]" , pdata->pvdd);
	...
	if(pdata->pvdd != -EINVAL)
	{
		...
		gpio_direction_input(pdata->pvdd);

		gpio_set_value(pdata->pvdd, 0);
		pr_info("nfc gpio_set_value PVDD ON\n");
	}
...
static int __sec_nfc_remove(struct device *dev)
{
	...
	{
		gpio_set_value(pdata->pvdd, 0);
		pr_info("nfc remove gpio_set_value PVDD Off\n");
		gpio_free(pdata->pvdd);
	}
