/* drivers/power/load_analyzer_msm8x26.c */
...
char cpu_bus_load_freq_menu[] ="==================================" \
			...
			"CPU0 ( C1/ C2/ C3)    CPU1   CPU2   CPU3   ONLINE   NR_RUN"
			"    BIMC     CNOC     PNOC      SNOC    GPUFREQ  GPU_UTIL\n";
					...
					, char *buf, unsigned int buf_size, int ret)
{
	...
	for (j = 0; j < show_cnt; j++) {
		...
		ret +=  snprintf(buf + ret, buf_size - ret
			,  "%8s    %c    %d.%d    %d.%d/%d.%d   %d/%d    [%5d]     "
			   "%3d (%3d/%3d/%3d)    %3d    %3d    %3d    "
			   "%3d     %3d.%02d    "
			   "%3d.%1d    %3d.%1d    %3d.%1d     %3d.%1d       %3d     %3d.%1d\n"
			, cpu_load_freq_history_view[cnt].time
