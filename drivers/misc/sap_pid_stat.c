...
static int __init sap_stat_init(void)
{
	...
	pe = proc_create("snd", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, parent, &sap_stat_snd_fops);
	if (!pe)
		goto fail_sap_stat_snd;
	pe = proc_create("rcv", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, parent, &sap_stat_rcv_fops);
	if (!pe)
		goto fail_sap_stat_rcv;

	pe = proc_create("snapshot", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, parent, &sap_stat_snapshot_fops);
	if (!pe)
		goto fail_sap_stat_snapshot;

	pe = create_proc_read_entry("stat", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, parent,
