...
static const struct max14577_muic_vps_data muic_vps_table[] = {
	...
	},
	{
		...
		.control1	= CTRL1_OPEN,
		.vps_name	= "500mA_TA",
		.attached_dev	= ATTACHED_DEV_500MA_TA_MUIC,
	},
	{
		.adcerr		= 0x00,
		.adclow		= (0x1 << STATUS1_ADCLOW_SHIFT),
		...
		.control1	= CTRL1_OPEN,
		.vps_name	= "2A_TA",
		.attached_dev	= ATTACHED_DEV_2A_TA_MUIC,
	},
};
			...
			enum muic_attached_dev new_dev)
{
	...
	if (muic_data->is_rustproof &&
			(tmp_vps->control1 == CTRL1_UART)) {
		pr_info("%s:%s rustproof!!, forcely prevent uart path control\n",
			MUIC_DEV_NAME, __func__);
...
static ssize_t max14577_muic_show_rustproof(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct max14577_muic_data *muic_data = dev_get_drvdata(dev);
...
static ssize_t max14577_muic_store_rustproof(struct device *dev,
			struct device_attribute *attr,
				const char *buf, size_t count)
{
	struct max14577_muic_data *muic_data = dev_get_drvdata(dev);
	...
	muic_data->is_rustproof = !!val;
	pr_info("%s: muic rustproof: %d\n", __func__, muic_data->is_rustproof);

	max14577_read_reg(muic_data->i2c, MAX14577_MUIC_REG_CONTROL2, &reg);
	pr_info("%s: CONTROL2(0x%02x) - before\n", __func__, reg);

	switch (muic_data->is_rustproof) {
		case 0:
			/* enable ADC detection */
			reg &= ~CTRL2_ADCEN_MASK;
			max14577_write_reg(muic_data->i2c, MAX14577_MUIC_REG_CONTROL2, reg);

			pr_info("%s:%s rustproof off!!, forcely connect uart path\n",
				MUIC_DEV_NAME, __func__);
			write_muic_ctrl_reg(muic_data, MAX14577_MUIC_REG_CONTROL1,
				CTRL1_UART);
			break;
		case 1:
			pr_info("%s:%s rustproof!!, forcely prevent uart path control\n",
	...
	max14577_read_reg(muic_data->i2c, MAX14577_MUIC_REG_CONTROL2, &reg);
	pr_info("%s: CONTROL2(0x%02x) - after\n", __func__, reg);
...
static DEVICE_ATTR(rustproof, 0664,
		max14577_muic_show_rustproof, max14577_muic_store_rustproof);
...
static void max14577_muic_attach_callback_jig(struct max14577_muic_data *muic_data)
{
	...
	if (muic_data->is_rustproof) {
		pr_info("%s:%s skip set_jig_state_cb() by rustproof\n",
			MUIC_DEV_NAME, __func__);
	} else {
	...
	case ATTACHED_DEV_USB_MUIC:
		...
		if (value.intval == SEC_WPC_DETECT_MUIC_INT) {
			...
			if (value.intval == POWER_SUPPLY_TYPE_WPC)
				...
				/* disable ldo2 not to use wpc ic */
				ret = max77836_ldo2_set(false);
				if (ret < 0)
					pr_err("%s: ldo2 set failed.\n", __func__);
				max14577_muic_attach_callback_usb(muic_data);
...
static int max14577_muic_irq_init(struct max14577_muic_data *muic_data)
{
	...
	max14577_bulk_read(muic_data->i2c, MAX14577_REG_INTMASK1, 3, intmask_val);
...
static int __devinit max14577_muic_probe(struct platform_device *pdev)
{
	...
	/* RUSTPROOF checking */
	ret = max14577_read_reg(muic_data->i2c,
			MAX14577_MUIC_REG_CONTROL1, &ctrl1);
	if (ret) {
		dev_err(&pdev->dev, "failed to read ctrl1\n");
		goto err_kfree;
	}

	ctrl1 &= (COMN1SW_MASK | COMP2SW_MASK);
	dev_info(&pdev->dev, "MUIC checked rust proof mode (0x%02x)\n", ctrl1);

	if (ctrl1 == CTRL1_RUSTPROOF_PATH)
	...
	/* Adjusted for Battery checking timing on Power-off charging state */
