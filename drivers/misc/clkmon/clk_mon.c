...
int clk_mon_get_clock_info(unsigned int *clk_status, char *buf)
{
	...
	clk_gate_mask_count = sizeof(clk_gate_masks) / sizeof(clk_gate_masks[0]);

	for (i = 0; (clk_gate_masks[i].addr != 0) && (i < clk_gate_mask_count); i++) {
		if (i >= bit_max) {
			pr_err("%s: Error Exceed storage size %d(%d)\n",
				__func__, i, bit_max);
			break;
