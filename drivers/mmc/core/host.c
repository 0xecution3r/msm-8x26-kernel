...
bool mmc_host_may_gate_card(struct mmc_card *card)
{
	...
	if(mmc_card_sdio(card)) return true;   // simply return true for SDIO
...
int mmc_add_host(struct mmc_host *host)
{
	...
	if (strcmp(host->parent->kobj.name, "msm_sdcc.2"))
		device_enable_async_suspend(&host->class_dev);
	else
		pr_info("%s: do not enable Async resume for msm_sdcc.2\n",
			mmc_hostname(host));
#else
	device_enable_async_suspend(&host->class_dev);
#endif
