	...
	struct mmc_ioc_rpmb __user *ic_ptr)
{
	...
	printk(KERN_WARNING"[MMC] RPMB Accessed by %s\n", current->comm);
	...
	if (mmc_card_mmc(card)) {
		...
		if (ret) {
			pr_err("%s: mmc_blk_part_switch failure, %d -> %d\n",
					mmc_hostname(card->host), main_md->part_curr, md->part_type);
			return ret;
		}
...
static int mmc_blk_issue_rq(struct mmc_queue *mq, struct request *req)
{
	...
	if (ret) {
		ret = mmc_blk_reset(md, card->host, MMC_BLK_PARTSWITCH);
		if (!ret)
		{
			...
			if (!ret)
			{
				pr_err("%s: mmc_blk_part_switch succeeded after reset.\n",
						mmc_hostname(host));
				 mmc_blk_reset_success(md, MMC_BLK_PARTSWITCH);
			}
			else pr_err("%s: mmc_blk_part_switch failed after reset.\n",
				mmc_hostname(host));
		}
		else pr_err("%s: mmc_blk_reset(MMC_BLK_PARTSWITCH) failed.\n",
			mmc_hostname(host));
	}
