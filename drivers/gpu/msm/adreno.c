 ...
 *
 * by optimization test result, 40ms is proper for 60fps
 * and 70ms is proper for 30fps.
 */
#define INPUT_WORK_DELAY 40
...
static void adreno_input_work(struct work_struct *work)
{
	...
	struct adreno_device *adreno_dev = container_of(d_work,
		...
		unsigned int code, int value)
{
	...
	if ((type == EV_ABS) &&
		...
		schedule_delayed_work(&adreno_dev->input_work, msecs_to_jiffies(INPUT_WORK_DELAY));
}
