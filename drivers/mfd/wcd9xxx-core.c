...
static int wcd9xxx_reset(struct wcd9xxx *wcd9xxx)
{
	...
	if (wcd9xxx->reset_gpio) {
		...
		if (wcd9xxx->reset_gpio == 55) {
			pr_info("%s: set tapan reset gpio as INPUT / PU\n",
								__func__);
			gpio_tlmm_config(GPIO_CFG(wcd9xxx->reset_gpio, 0,
...
#ifdef CONFIG_MACH_WC1
		if (wcd9xxx->reset_gpio == 55) {
			pr_info("%s: set tapan reset gpio as OUTPUT / NP\n",
								__func__);
			gpio_tlmm_config(GPIO_CFG(wcd9xxx->reset_gpio, 0,
