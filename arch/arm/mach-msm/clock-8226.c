...
#define MMSS_GP1_M                                  (0x3458)
#define MMSS_GP1_N                                  (0x345C)
...
#if defined(CONFIG_MACH_WC1) || defined(CONFIG_MACH_NC1)
unsigned int MMSS_GP1_D_read(void)
{
	u32 value;

	value = readl_relaxed(MMSS_REG_BASE(MMSS_GP1_M));
	/* printk("M Reg(%08x) : %d\n", (int)MMSS_REG_BASE(MMSS_GP1_M), value); */

	value = readl_relaxed(MMSS_REG_BASE(MMSS_GP1_N));
	/* printk("N Reg(%08x) : %d\n", (int)MMSS_REG_BASE(MMSS_GP1_N), value); */

	value = readl_relaxed(MMSS_REG_BASE(MMSS_GP1_D));
	/* printk("D Reg(%08x) : %d\n", (int)MMSS_REG_BASE(MMSS_GP1_D), value); */

	return value;
}

void MMSS_GP1_D_write(void)
{
	u32 value;

	value = readl_relaxed(MMSS_REG_BASE(MMSS_GP1_N));
	writel_relaxed(value, MMSS_REG_BASE(MMSS_GP1_N));
	writel_relaxed(200, MMSS_REG_BASE(MMSS_GP1_D));
}

unsigned int MMSS_GP1_cmd_RCGR(void)
{
	unsigned int value;

	writel_relaxed(0x3, MMSS_REG_BASE(MMSS_GP1_CMD_RCGR));
	value = readl_relaxed(MMSS_REG_BASE(MMSS_GP1_CMD_RCGR));
...
static struct clk_lookup msm_clocks_8226[] = {
	...
	/* NFC */
	CLK_LOOKUP("nfc_clk",            cxo_d1_pin.c, "2-0027"),
	/* PIL-PRONTO */
...
#if defined(CONFIG_SND_SOC_ES705) || defined(CONFIG_MSM8226_UART4)
...
#if defined(CONFIG_MACH_WC1) || defined(CONFIG_MACH_NC1)
	CLK_LOOKUP("isa1000a_clk", gcc_gp1_clk.c, "motor_pwm"),
#else
	...
	CLK_LOOKUP("mmss_s0_axi_clk", mmss_s0_axi_clk.c, NULL),
...
unsigned int get_cnoc_clk(void)
{
	u64 data;
	int ret;
	ret=clock_debug_measure_get_extern(&(cnoc_clk.c) , &data);
	if(ret<0)
	pr_err("%s --cannot measure clock rate ",__func__);
	return (unsigned int)data;
}
...
unsigned int get_pnoc_clk(void)
{
	u64 data;
	int ret;
	ret=clock_debug_measure_get_extern(&(pnoc_clk.c) , &data);
	if(ret<0)
	pr_err("%s --cannot measure clock rate ",__func__);
	return (unsigned int)data;
}
