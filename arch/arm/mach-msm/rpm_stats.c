...
#ifdef CONFIG_VMIN_DEBUG
#define MAX_NUM_COUNT 6
u32 count_history[MAX_NUM_COUNT];
u32 modem_history[MAX_NUM_COUNT];
int count_idx = 0;
int vmin_error=0;
extern int rpm_mpss_sleep_monitor_read_temp(void);
extern void clock_debug_print_enabled_temp(void);
...
#ifdef CONFIG_VMIN_DEBUG
	/* Save VMIN information only */
	if (mode_idx == 1) {
		count_history[count_idx % MAX_NUM_COUNT] = count;
		modem_history[count_idx % MAX_NUM_COUNT] = rpm_mpss_sleep_monitor_read_temp();

		printk("wook: %d, vmin=[%d], modem=[%08X]\n", count_idx, count_history[count_idx % MAX_NUM_COUNT], modem_history[count_idx % MAX_NUM_COUNT]);

		/* Check only if count_idx is greater than MAX_NUM_COUNT */
		if (count_idx > MAX_NUM_COUNT) {
			int i;
			for (i = 1; i < MAX_NUM_COUNT; i++) {

				if (count_history[0] != count_history[i % MAX_NUM_COUNT]) {
					printk("wook: break\n");
					break;
				}

				/* If modem is enabled, it is OK */
				if ((modem_history[i] & 0xf0000000)>> 28 == 1) {
					break;
				}

				if (i == MAX_NUM_COUNT -1)
					vmin_error++;
			}

			/* When vmin_error is detcted, do more actions what you want */
			if ((vmin_error % 100) == 1 || (vmin_error % 100) == 2)	{

				clock_debug_print_enabled_temp();
				//panic("vmin error! count_idx=%d, count=%d\n", count_idx, count);
			}
