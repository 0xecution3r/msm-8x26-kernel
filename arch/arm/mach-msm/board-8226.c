...
#elif defined (CONFIG_MACH_WJ1) || defined (CONFIG_MACH_WC1)\
...
#elif defined (CONFIG_MACH_WC1) || defined(CONFIG_MACH_NC1)
static int __init sensor_hub_init(void)
{
	struct regulator *vdd_sensors, *vdd_hrm, *vdd_hub, *vdd_hrm_io;
	int ret;
	...
	gpio_set_value(GPIO_MCU_RST, 0);

	/* MCU power enable */
	/* ~ rev05: LVS1 */
	/* rev06 ~: VREG_L6(boot on) from system_rev 06 */
	/* rev06 ~: LVS1 used for HALL IC temperally on*/
	...
	vdd_sensors = regulator_get(NULL, "8226_l27");
	if (IS_ERR(vdd_sensors)) {
		pr_err("[SSP] %s, could not get 8226_l27(sensors vdd), %ld\n",
			__func__, PTR_ERR(vdd_sensors));
	} else {
		ret = regulator_set_voltage(vdd_sensors, 1800000, 1800000);
		if (ret)
			pr_err("[SSP] %s, fail to set voltage for 8226_l27(sensors vdd)\n",
				__func__);
		ret = regulator_enable(vdd_sensors);
		if (ret)
			pr_err("[SSP] %s, can't turn on for 8226_l27(sensors vdd)\n", __func__);
	}

	vdd_hrm = regulator_get(NULL, "8226_l16");
	if (IS_ERR(vdd_hrm)) {
		pr_err("[SSP] %s, could not get 8226_l16(hrm vdd), %ld\n",
			__func__, PTR_ERR(vdd_hrm));
	} else {
		if (system_rev < 0x5)
			ret = regulator_set_voltage(vdd_hrm, 1800000, 1800000);
		else
			ret = regulator_set_voltage(vdd_hrm, 3000000, 3000000);
		if (ret)
			pr_err("[SSP] %s, fail to set voltage for 8226_l16\n",
				__func__);

		ret = regulator_enable(vdd_hrm);
		if (ret)
			pr_err("[SSP] %s, can't turn on for 8226_l16\n",
				__func__);
	}

	/* HRM_IO: use VREG_L18 from system_rev 07*/
	if ((system_rev == 0x7) || (system_rev == 0x8)) {
		vdd_hrm_io = regulator_get(NULL, "8226_l18");
		if (IS_ERR(vdd_hrm_io)) {
			pr_err("[SSP] %s, could not get 8226_l18(vdd_hrm_io), %ld\n",
				__func__, PTR_ERR(vdd_hrm_io));
		} else {
			ret = regulator_set_voltage(vdd_hrm_io, 1800000, 1800000);
			if (ret)
				pr_err("[SSP] %s, fail to set voltage for 8226_l18(vdd_hrm_io)\n",
					__func__);
			ret = regulator_enable(vdd_hrm_io);
			if (ret)
				pr_err("[SSP] %s, can't turn on for 8226_l18(vdd_hrm_io)\n",
					__func__);
		}
	}
...
static int __init dbmd2_init(void)
{
	struct regulator *dbmd2_vdd_core;
	int ret;

	pr_info("%s\n", __func__);

	dbmd2_vdd_core = regulator_get(NULL, "8226_l26");
	if (IS_ERR(dbmd2_vdd_core)) {
		pr_err("%s, could not get 8226_l26(dbmd2_vdd_core), %ld\n",
			__func__, PTR_ERR(dbmd2_vdd_core));
	} else {
		ret = regulator_set_voltage(dbmd2_vdd_core, 1200000, 1200000);
		if (ret)
			pr_err("%s, fail to set voltage for 8226_l26(dbmd2_vdd_core)\n",
				__func__);
		ret = regulator_enable(dbmd2_vdd_core);
		if (ret)
			pr_err("%s, can't turn on for 8226_l26(dbmd2_vdd_core)\n", __func__);
	}
