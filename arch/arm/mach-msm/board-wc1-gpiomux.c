...
#if defined(CONFIG_MACH_WC1_CDMA_SPR) || defined(CONFIG_MACH_WC1_CDMA_VZW)
static struct gpiomux_setting adj_det_cfg = {
...
static struct msm_gpiomux_config cdma_adj_det_cfg[] = {
	{
		.gpio = 79,
		.settings = {
			[GPIOMUX_ACTIVE] = &adj_det_cfg,
			[GPIOMUX_SUSPENDED] = &adj_det_cfg,
		},
	},
};
#endif
...
static struct msm_gpiomux_config wc1_hw_check_additional_bit_cfg[] = {
	{
		.gpio = 55,
		.settings = {
			[GPIOMUX_ACTIVE] = &hw_check_bit_cfg,
...
static struct msm_gpiomux_config wc1_haptic_configs[]={
	{
		.gpio = 40,
		.settings = {
			[GPIOMUX_ACTIVE] = &haptic_temp_cofig,
...
static struct gpiomux_setting pma_int_cofig ={
	.func = GPIOMUX_FUNC_GPIO,
	.drv = GPIOMUX_DRV_2MA,
	.pull = GPIOMUX_PULL_NONE,
	.dir = GPIOMUX_IN,
...
static struct msm_gpiomux_config wc1_pma_int_cfg[]={
	{
		.gpio = 67,
		.settings = {
			[GPIOMUX_ACTIVE] = &pma_int_cofig,
			[GPIOMUX_SUSPENDED] = &pma_int_cofig,
		},
	},
};
...
static struct gpiomux_setting dischg_standby_cfg = {
	.func = GPIOMUX_FUNC_GPIO,
	.drv = GPIOMUX_DRV_2MA,
	.dir = GPIOMUX_IN,
	.pull = GPIOMUX_PULL_DOWN,
...
static struct gpiomux_setting dischg_sus_cfg = {
	.func = GPIOMUX_FUNC_GPIO,
	.drv = GPIOMUX_DRV_2MA,
	.pull = GPIOMUX_PULL_DOWN,
};
...
static struct msm_gpiomux_config msm_dischg_configs[] __initdata = {
	{
		.gpio = 36,		/* DISCHG */
		.settings = {
			[GPIOMUX_ACTIVE] = &dischg_standby_cfg,
			[GPIOMUX_SUSPENDED] = &dischg_sus_cfg,
		},
	},
};
...
static struct msm_gpiomux_config msm_tapan_config55[] __initdata = {
	{
		.gpio	= 68,		/* CDC_INT */
		.settings = {
...
void __init msm8226_init_gpiomux(void)
{
	...
	if (system_rev >= 6)
		...
		msm_gpiomux_install(msm_tapan_config55,
			ARRAY_SIZE(msm_tapan_config55));
#if 0
	...
	msm_gpiomux_install(b3_haptic_configs, ARRAY_SIZE(b3_haptic_configs));
#else
	msm_gpiomux_install(wc1_haptic_configs, ARRAY_SIZE(wc1_haptic_configs));
#endif
	...
	if (system_rev >= 6)
		msm_gpiomux_install(wc1_hw_check_additional_bit_cfg,
					ARRAY_SIZE(wc1_hw_check_additional_bit_cfg));
#if defined(CONFIG_SND_SOC_DBMD2) || defined(CONFIG_SND_SOC_DBMD2P3)
	...
	if (system_rev < HW_REV_02)
		msm_gpiomux_install(msm_ofm_configs,
				ARRAY_SIZE(msm_ofm_configs));

	if (system_rev >= HW_REV_04)
		msm_gpiomux_install(msm_dischg_configs,
				ARRAY_SIZE(msm_dischg_configs));

	if (system_rev >= 8)
		msm_gpiomux_install(wc1_pma_int_cfg, ARRAY_SIZE(wc1_pma_int_cfg));

	msm_gpiomux_install(msm_tsp_int_config, ARRAY_SIZE(msm_tsp_int_config));
#if defined(CONFIG_MACH_WC1_CDMA_SPR) || defined(CONFIG_MACH_WC1_CDMA_VZW)
	msm_gpiomux_install(cdma_adj_det_cfg, ARRAY_SIZE(cdma_adj_det_cfg));
#endif
}
