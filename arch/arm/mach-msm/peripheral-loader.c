...
static int pil_setup_region(struct pil_priv *priv, const struct pil_mdt *mdt)
{
	...
	pil_info(priv->desc, "%srelocate start=%08x, end=%08x\n",
			relocatable == true ? "" : "non-",
	...
	if (seg->filesz) {
		...
		if (ret < 0) {
			pil_err(desc, "Failed to locate blob %s or blob is too big.\n",
				fw_name);
			pil_err(desc, "ret [%d], seg size %lu\n", ret, seg->filesz);
			usleep_range(10000, 11000);
			ret = request_firmware_direct(fw_name, desc->dev,
					seg->paddr, seg->filesz);
			if (ret < 0) {
				pil_err(desc, "2nd trial has failed to located blob %s %d\n",
						fw_name, ret);
#ifdef CONFIG_SLP_KERNEL_ENG
				return ret;
#else
				panic("%s loading failure\n", fw_name);
#endif
			} else {
				pil_err(desc, "2nd trial has success %s\n", fw_name);
			}
		}
...
int pil_boot(struct pil_desc *desc)
{
	...
	if (ret) {
		int retry = 0;
		pil_err(desc, "Failed to locate %s. ret %d\n", fw_name, ret);

		for (retry = 0; retry < 3; retry++) {
			usleep_range(10000, 11000);
			ret = request_firmware(&fw, fw_name, desc->dev);
			if (ret)
				pil_err(desc, "[%d] Failed to locate %s. ret %d\n",
							retry, fw_name, ret);
			else
				break;
		}
		...
		if (retry == 3) {
			pil_err(desc, "doesn't recover with 3 retry\n");
#ifdef CONFIG_SLP_KERNEL_ENG
			goto out;
#else
			panic("%s loading failure\n", fw_name);
			...
			pil_err(desc, "%s loading recover success\n", fw_name);
		}
	}
