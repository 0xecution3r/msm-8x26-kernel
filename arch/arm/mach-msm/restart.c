...
static void __msm_power_off(int lower_pshold)
{
	...
	/* Workaround routine to do dvdd shutdown instead of shutdown when TA(USB) is connected */
	if (! poweroff_charging && (current_cable_type == POWER_SUPPLY_TYPE_MAINS || current_cable_type == POWER_SUPPLY_TYPE_USB)) {
		printk(KERN_EMERG "Powering off the SoC, charging now. do dvdd_shutdown\n");
