...
#define XPU_ERROR_ADDRESS 0xfc00000
...
static void simulate_xpu_error(void)
{
	uint32_t address = XPU_ERROR_ADDRESS;
	void *xpu_error_address = NULL;

	xpu_error_address = ioremap(address, SZ_4K);
	writel_relaxed(0x1111, xpu_error_address);
	mdelay(DELAY_TIME);
	/* if we hit here, test had failed */
	pr_emerg("XPU error test failed...\n");
	iounmap(xpu_error_address);
...
static int force_error(const char *val, struct kernel_param *kp)
{
	...
	if (!strncmp(val, "appdogbark", 10)) {
		...
		*(unsigned int *)0x0 = 0x0;
		...
		simulate_bus_hang();
	} else if (!strncmp(val, "xpuerr", 6)) {
		pr_emerg("Generating a XPU error!\n");
		simulate_xpu_error();
