...
static int __scm_call(const struct scm_command *cmd)
{
	 ...
	 */
	call_from_ss_daemon = (strncmp(current_thread_info()->task->comm,
			"secure_storage_daemon", TASK_COMM_LEN - 1) == 0);
	...
	/*
	 * The samsung secure storage need to access code segment, so the
	 * flush_cache_call is needed before switching secure world to guarantee
	 * secure storage operation. Furthermore it is required for each cpu
	 * according to cache mechanism.
	 */
...
#ifndef CONFIG_ARCH_MSM_KRAIT
		/*
		 * Krait core doesn't need to flush cache all for each CPU
		 */
		smp_call_function(__wrap_flush_cache_all, NULL, 1);
#endif
