...
struct msm_rpm_master_stats_platform_data *gv_pdata;
#endif
...
int rpm_ee_sleep_monitor_read(void *priv, unsigned int *raw_val, char * ee_name)
{
	...
	for (master_cnt = 0; master_cnt <= pdata->num_masters ; master_cnt++)
		if(!strncmp(ee_name, pdata->masters[master_cnt], sizeof(ee_name)))
			break;
...
#ifdef CONFIG_VMIN_DEBUG
int rpm_mpss_sleep_monitor_read_temp(void)
{
	struct msm_rpm_master_stats_platform_data *pdata;
	unsigned int master_cnt;
	uint32_t active_cores;
	...
	/* Find valid index for given  master */
	for (master_cnt = 0; master_cnt <= pdata->num_masters ; master_cnt++)
		if(!strncmp("MPSS", pdata->masters[master_cnt], 4))
			break;
	...
	return (active_cores << 28) | (0x0fffffff & numshutdowns);
}
EXPORT_SYMBOL_GPL(rpm_mpss_sleep_monitor_read_temp);
...
int rpm_lpss_sleep_monitor_read(void *priv, unsigned int *raw_val,
		int check_level, int caller_type)
{
	return rpm_ee_sleep_monitor_read(priv, raw_val, "LPSS");
}
...
static struct sleep_monitor_ops rpm_lpss_sleep_monitor_ops = {
	.read_cb_func = rpm_lpss_sleep_monitor_read,
};
#endif
...
static  int __devinit msm_rpm_master_stats_probe(struct platform_device *pdev)
{
	...
	if (pdata->version == 2) {
		...
		sleep_monitor_register_ops(pdata, &rpm_lpss_sleep_monitor_ops, SLEEP_MONITOR_EE_LPSS);
#ifdef CONFIG_VMIN_DEBUG
	gv_pdata = pdata;
#endif
	}
