...
#ifdef CONFIG_SND_SOC_MSM8226
	/*
	 * To make dbmd2 card as secondary card (e.g. hw1:0)
	 * Add routine to check main codec has ready.
	 */
	if (!msm8226_get_tapan_ready()) {
		dev_dbg(&pdev->dev, "%s(): tapan is not ready\n", __func__);
		return -EPROBE_DEFER;
	} else {
		dev_dbg(&pdev->dev, "%s(): tapan is ready\n", __func__);
	}
#endif
