...
#if defined(CONFIG_MACH_B3) || defined(CONFIG_MACH_WJ1)
#include <mach/sec_audio.h>
#endif

#if defined(FEATURE_PDM_MIC_DHWPT)
#define ES705_PDM_MIC_DHWPT_BTOD	0x905201DD /* from Port B to Port D CLK, Unidirectional */
#define ES705_PDM_MIC_DHWPT_ATOC	0x905201C8 /* from Port A to Port C CLK, Unidirectional */
#define ES705_PDM_MIC_DHWPT_RESET	0x90520000 /* reset the all active passthrough routes */
#endif
...
struct es705_priv es705_priv = {
	...
	.keyword_type = 0,
#endif
#if defined(FEATURE_STAND_ALONE_VS_FW)
	.stand_alone_vs_fw_enable = 0,
#endif
#endif
...
#if defined(FEATURE_PDM_MIC_DHWPT)
int es705_register_mic_bias_en_cb(void (*micbias_en_cb_fn)(int en, int src))
{
	int ret = 0;
	if (micbias_en_cb_fn) {
		es705_priv.micbias_en_cb = micbias_en_cb_fn;
		pr_info("%s(): micbias_en_cb registered 0x%p\n",
					__func__, micbias_en_cb_fn);
	} else {
		es705_priv.micbias_en_cb = NULL;
		ret = -1;
		pr_warn("%s(): NULL micbias_en_cb_fn\n", __func__);
	}

	return ret;
}
EXPORT_SYMBOL(es705_register_mic_bias_en_cb);
#endif
...
#if defined(FEATURE_PDM_MIC_DHWPT)
static int es705_pdm_mic_dhwpt(struct es705_priv *es705,
					int power_state, int onoff);
#endif
	...
	if (blocks) {
		...
		if (es705->rdb_wdb_open) {
			...
			do {
				...
				dev_info(es705->dev, "%s(): v-s wdb %d bytes written\n",
					__func__, sz);
...
static void es705_write_sensory_vs_data_block(int type)
{
	...
	dev_dbg(es705_priv.dev, "%s() size %d bytes\n", __func__, size);
	...
	rc = es705_write_then_read(&es705_priv, &check_cmd, sizeof(check_cmd),
					 &check_rspn, 0);
	dev_info(es705_priv.dev, "%s(): size = %x\n", __func__, check_rspn);
...
static int es705_write_sensory_vs_keyword(void)
{
...
#if defined(FEATURE_STAND_ALONE_VS_FW)
	/* if 3 is 3Mbps, 4 is 1.5Mbps and 5 is 1Mbps */
	...
	if (rc) {
		...
		dev_info(es705_priv.dev, "%s : request_firmware the grammar bin\n", __func__);
	...
	if (rc) {
		...
		dev_info(es705_priv.dev, "%s : request_firmware the net bin\n", __func__);

	/* download keyword using WDB */
	es705_write_sensory_vs_data_block(0);
#ifdef GEAR3_DEBUG
	/* verify the download size nd the CRC */
	check_cmd = 0x8016e031;
	check_rspn = 0;
	rc = es705_write_then_read(&es705_priv, &check_cmd, sizeof(check_cmd),
					 &check_rspn, 0);
	dev_dbg(es705_priv.dev,
			"%s: grammar size cmd rspn = 0x%x\n", __func__, check_rspn);
	...
	rc = es705_write_then_read(&es705_priv, &check_cmd, sizeof(check_cmd),
					 &check_rspn, 0);
	dev_dbg(es705_priv.dev,
			"%s: grammar crc cmd rspn = 0x%x\n", __func__, check_rspn);
#endif


	es705_write_sensory_vs_data_block(1);
#ifdef GEAR3_DEBUG
	/* verify the download size and the CRC */
	check_cmd = 0x8016e031;
	check_rspn = 0;
	rc = es705_write_then_read(&es705_priv, &check_cmd, sizeof(check_cmd),
					 &check_rspn, 0);
	dev_dbg(es705_priv.dev,
			"%s: net size cmd rspn = 0x%x\n", __func__, check_rspn);
				...
				const char *buf, size_t count)
{
	...
	if (index)
		...
		dev_dbg(es705_priv.dev, "%s : [ES705] doesn't /\n", __func__);
...
static int es705_fw_download(struct es705_priv *es705, int fw_type)
{
	...
	dev_info(es705->dev, "%s(): fw download type %s begin\n",
...
static void es705_get_default_keyword(void)
{
	memset(es705_priv.grammar_path, 0, sizeof(es705_priv.grammar_path));
	/*
	sprintf(es705_priv.grammar_path, "search_20.bin");
	sprintf(es705_priv.grammar_path, "higalaxy_en_us_gram6.bin");
	sprintf(es705_priv.grammar_path, "higear_search_en_us.bin");
	*/
	sprintf(es705_priv.grammar_path, "higear_search");
	dev_info(es705_priv.dev, "%s(): default keyword grammar path %s\n",
					__func__, es705_priv.grammar_path);

	memset(es705_priv.net_path, 0, sizeof(es705_priv.net_path));
	/*
	sprintf(es705_priv.net_path, "net.bin");
	sprintf(es705_priv.net_path, "higalaxy_en_us_am.bin");
	sprintf(es705_priv.net_path, "higear_am_en_us.bin");
	*/
	sprintf(es705_priv.net_path, "higear_am");
...
int es705_bootup(struct es705_priv *es705)
{
	...
	if (rc) {
		...
		mutex_lock(&es705->pm_mutex);
#if defined(FEATURE_STAND_ALONE_VS_FW)
		/* Forced set default keyword firmware for testing */
		es705_get_default_keyword();

		rc = es705_write_sensory_vs_keyword();
		if (rc)
			dev_err(es705->dev,
				"%s(): keyword download FAIL\n", __func__);
		else
			dev_info(es705->dev,
				"%s(): keyword download SUCCESS\n", __func__);
#endif
		es705->pm_state = ES705_POWER_AWAKE;
#if defined(SAMSUNG_ES705_FEATURE)
		es705->es705_power_state = ES705_SET_POWER_STATE_NORMAL;
...
static int es705_sleep(struct es705_priv *es705)
{
	...
	if (es705->es705_power_state == ES705_SET_POWER_STATE_SLEEP) {
		dev_err(es705->dev, "%s(): no need to go to Sleep Mode\n", __func__);		
		return rc;
	}
...
#if defined(FEATURE_PDM_MIC_DHWPT)
	dev_info(es705->dev, "%s(): pdm_mic_dhwpt : %d\n",
				__func__, es705->pdm_mic_dhwpt);	
	if (!(es705->pdm_mic_dhwpt)) {
		es705_pdm_mic_dhwpt(es705, 0, 1);
	}
#endif
...
#if defined(FEATURE_SLEEP_BY_UART)
static int es705_sleep_by_uart(struct es705_priv *es705)
{
	int rc = 0;

#if defined(FEATURE_AUDIENCE_POWER_EXP_2ND)
	char wakeup_char = 'A';
#endif
	u32 cmd;
	...
	dev_info(es705->dev, "%s(): pdm_mic_dhwpt : %d\n",
				__func__, es705->pdm_mic_dhwpt);
	if (!(es705->pdm_mic_dhwpt)) {
		if (es705->micbias_en_cb) {
			es705->micbias_en_cb(0, SEC_MICBIAS_SRC_VS);
		}

		cmd = ES705_PDM_MIC_DHWPT_BTOD;
		cmd = cpu_to_be32(cmd);
		rc = es705_uart_write(es705, (char*)&cmd, sizeof(cmd));
		if (rc < 0)
			dev_err(es705->dev, "%s(): Failed PDM DHWPT B to D", __func__);

		cmd = ES705_PDM_MIC_DHWPT_ATOC;
		cmd = cpu_to_be32(cmd);
		rc = es705_uart_write(es705, (char*)&cmd, sizeof(cmd));
		if (rc < 0)
			dev_err(es705->dev, "%s(): Failed PDM DHWPT A to C", __func__);
		else
			es705->pdm_mic_dhwpt = 1;
	}
#endif
	...
	/*
	* write Set Power State Sleep - 0x9010_0001
	* wait 30 ms, and then turn ext clock off
	* There will not be any response after
	* sleep command from chip
	*/
	cmd = (ES705_SET_POWER_STATE << 16) | ES705_SET_POWER_STATE_SLEEP;
...
es705_sleep_by_uart_exit:
	es705_uart_close(es705);
	dev_info(es705->dev, "%s(): Exit rc = %d\n",__func__, rc);
	mutex_unlock(&es705->pm_mutex);
	return rc;
...
#if !defined(CONFIG_SND_SOC_ES705_STAND_ALONE_VS_FW)
	if (es705->es705_power_state != ES705_SET_POWER_STATE_VS_LOWPWR &&
		es705->es705_power_state != ES705_SET_POWER_STATE_VS_OVERLAY)
		es705_switch_to_normal_mode(&es705_priv);
#endif
...
#if defined(FEATURE_STAND_ALONE_VS_FW)
/* Function to be called on AP wakeup to wakeup the es705 chip */
static int esxxx_wakeup_chip(void)
{
	int rc = 0;
	u32 cmd_resp;
	u32 sync_cmd;
	...
	u32 vs_keyword_length_cmd = 0x902C0000
				| (es705_priv.vs_keyword_length & 0xFFFF);
#endif
#if defined(FEATURE_SENSORY_EXP)
#ifdef GEAR3_DEBUG
	u32 check_cmd;
	u32 check_rspn;
#endif
	u32 cmd_lpsd[] = {0x9017e013, 0x90180001, /* Enable LPSD */
			 /*  0x9017e03c, 0x90180000, */ /* LPSD Min Threshold 0 */
			 /* 0x9017e045, 0x90180001,*/ /* Clock switching enable */
			  0xffffffff};
	u32 cmd_threshold[] = {
			  0x9017e049, 0x901800c8, /* 200 */
			  0xffffffff};
#endif

#if defined(FEATURE_AUDIENCE_POWER_EXP)
	/* new power level to be used for power optimization */
	u32 vs_power_level7 = 0x90110007;
#endif
#if defined(FEATURE_AUDIENCE_POWER_EXP_2ND)
	/*
	* Added support for VS low power mode
	* 512K PDM(19MIPS), UART 1Mbps
	*/
	u32 vs_low_power_state = (ES705_SET_POWER_STATE << 16) |
								ES705_SET_POWER_STATE_VS_LOWPWR;
#endif
	/* Overlay mode doesn't need wakeup */
	if (es705_priv.es705_power_state !=
			ES705_SET_POWER_STATE_SLEEP) {
		dev_err(es705_priv.dev, "%s(): chip is not in sleep",
		__func__);
		goto esxxx_wakeup_chip_exit;
	}

	rc = es705_wakeup(&es705_priv);
	if (rc) {
		dev_err(es705_priv.dev, "%s(): wakeup failed", __func__);
		goto esxxx_wakeup_chip_exit;
	}
	es705_priv.pm_state = ES705_POWER_AWAKE;
...
#if defined(FEATURE_PDM_MIC_DHWPT)
	dev_info(es705_priv.dev, "%s(): pdm_mic_dhwpt : %d\n",
				__func__, es705_priv.pdm_mic_dhwpt);	
	if (es705_priv.pdm_mic_dhwpt) {
		es705_pdm_mic_dhwpt(&es705_priv, 0, 0);
	}
#endif
	...
	/* Enable Voice Sense Event INTR to Host */
	rc = es705_priv.cmd(&es705_priv, ES_SET_EVENT_RESP, 0, &cmd_resp);
	if (rc) {
		dev_err(es705_priv.dev, "%s(): sending ES_SET_EVENT_RESP on slim failed\n",
				__func__);
		goto esxxx_wakeup_chip_exit;
	} else {
		dev_info(es705_priv.dev, "%s(): sending ES_SET_EVENT_RESP and resp is 0x%08x\n",
				__func__, cmd_resp);
	}
	sync_cmd = (ES705_SYNC_CMD << 16) | ES705_SYNC_INTR_RISING_EDGE;		
	rc = es705_priv.cmd(&es705_priv, sync_cmd, 0, &cmd_resp);
	if (rc) {
		dev_err(es705_priv.dev, "%s(): sending sync_cmd(0x%08x) on slim failed\n",
				__func__, sync_cmd);
		goto esxxx_wakeup_chip_exit;
	} else {
		dev_info(es705_priv.dev, "%s(): sending sync_cmd and resp is 0x%08x\n",
				__func__, cmd_resp);
	}
#if defined(FEATURE_SENSORY_EXP)
	rc = es705_write_block(&es705_priv, cmd_lpsd);
	if (rc)
		dev_err(es705_priv.dev,
			"%s(): LPSD min threshold FAIL\n", __func__);
	else
		dev_info(es705_priv.dev,
			"%s(): LPSD min threshold SUCCESS\n", __func__);

	rc = es705_write_block(&es705_priv, cmd_threshold);
	if (rc)
		dev_err(es705_priv.dev,
			"%s(): threshold set FAIL\n", __func__);
	else
		dev_info(es705_priv.dev,
			"%s(): threshold set SUCCESS\n", __func__);

#ifdef GEAR3_DEBUG
	check_cmd = 0x8016e013;
	check_rspn = 0;
	rc = es705_write_then_read(&es705_priv, &check_cmd, sizeof(check_cmd),
						&check_rspn, 0);
	if (rc)
		dev_err(es705_priv.dev, "%s(): 0x%x Failed\n",
						__func__, check_cmd);
	else
		dev_dbg(es705_priv.dev, "%s(): 0x%x Success (0x%X)\n",
					__func__, check_cmd, check_rspn);
	check_cmd = 0x8016e03c;
	check_rspn = 0;
	rc = es705_write_then_read(&es705_priv, &check_cmd, sizeof(check_cmd),
						&check_rspn, 0);
	if (rc)
	...
	/* new power level to be used for power optimization */
	es705_cmd(&es705_priv, vs_power_level7);
	dev_info(es705_priv.dev, "%s(): send power level to 7(0x%08x)\n",
			__func__, vs_power_level7);
#endif

#if defined(FEATURE_AUDIENCE_POWER_EXP_2ND)
	es705_cmd(&es705_priv, vs_low_power_state);
	dev_info(es705_priv.dev, "%s(): send power state to 6(0x%08x)\n",
			__func__, vs_low_power_state);
#endif


	/* Add event type */
	es705_priv.voice_wakeup_enable = 1;

esxxx_wakeup_chip_exit:
	return rc;
}

static int esxxx_sleep_chip(void)
{
	int rc = 0;

#if defined(FEATURE_SLEEP_BY_UART)
	/* Change sleep cmd intf from slimbus to uart */
	rc = es705_sleep_by_uart(&es705_priv);
#else
#if defined(FEATURE_AUDIENCE_POWER_EXP_2ND)
	if (es705_priv.es705_power_state == ES705_SET_POWER_STATE_VS_LOWPWR) {
		rc = es705_uart_es705_wakeup(&es705_priv);
		if (rc < 0) {
			goto esxxx_sleep_chip_exit;
		} else {
			es705_switch_to_normal_mode(&es705_priv);
			msleep(100);
		}
...
#endif
	rc = es705_sleep(&es705_priv);
#endif

	if (rc) {
		dev_err(es705_priv.dev, "%s(): sleep failed", __func__);
		goto esxxx_sleep_chip_exit;
	}

esxxx_sleep_chip_exit:
	es705_priv.internal_route_num = 40;
	return rc;
}

static int esxxx_get_stand_alone_vs_fw(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = es705_priv.stand_alone_vs_fw_enable;
	dev_dbg(es705_priv.dev, "%s(): stand_alone_vs_fw_enable = %d\n",
		__func__, es705_priv.stand_alone_vs_fw_enable);

	return 0;
}

static int esxxx_put_stand_alone_vs_fw(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	int rc = 0;

	if (es705_priv.stand_alone_vs_fw_enable == ucontrol->value.integer.value[0]) {
		dev_info(es705_priv.dev, "%s(): skip to set stand_alone_vs_fw_enable[%d->%ld]\n",
				__func__, es705_priv.stand_alone_vs_fw_enable,
				ucontrol->value.integer.value[0]);
		return rc;
	}
	es705_priv.stand_alone_vs_fw_enable = ucontrol->value.integer.value[0];
	dev_info(es705_priv.dev, "%s(): stand_alone_vs_fw_enable = %d\n",
				__func__, es705_priv.stand_alone_vs_fw_enable);

	if (es705_priv.stand_alone_vs_fw_enable == 1) {
		rc = esxxx_wakeup_chip();
		if (rc)
			es705_priv.stand_alone_vs_fw_enable = 0;
	} else {
		rc = esxxx_sleep_chip();
		if (rc)
			es705_priv.stand_alone_vs_fw_enable = 1;
	}

	if (rc)
		dev_info(es705_priv.dev, "%s(): failed rc = %d\n",
					__func__, rc);
	return rc;
}

static int esxxx_get_update_vs_keyword(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
static int esxxx_put_update_vs_keyword(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	int rc = 0;

	if (ucontrol->value.integer.value[0]) {
		dev_info(es705_priv.dev, "%s(): entry\n", __func__);
		/* wake up the chip */
		if (es705_priv.es705_power_state
				!= ES705_SET_POWER_STATE_SLEEP) {
			dev_err(es705_priv.dev,
				"%s(): chip is not in sleep", __func__);
			goto esxxx_update_vs_keyword_exit;
		}

		rc = es705_wakeup(&es705_priv);
		if (rc) {
			dev_err(es705_priv.dev,
				"%s(): wakeup failed", __func__);
			goto esxxx_update_vs_keyword_exit;
		}
		es705_priv.pm_state = ES705_POWER_AWAKE;
		es705_priv.es705_power_state = ES705_SET_POWER_STATE_NORMAL;
		...
		/* Download Keyword and Model binary through WDB */
		mutex_lock(&es705_priv.pm_mutex);
		if (!es705_priv.vs_grammar_set_flag
				|| !es705_priv.vs_net_set_flag)
			es705_get_default_keyword();
...
esxxx_update_vs_keyword_exit:
	dev_info(es705_priv.dev, "%s(): exit rc = %d\n", __func__, rc);
	return rc;
}
#endif

#if defined(FEATURE_PDM_MIC_DHWPT)
static int es705_pdm_mic_dhwpt(struct es705_priv *es705, int power_state, int onoff)
{
	int rc = 0;
	u32 cmd = (ES705_SYNC_CMD << 16) | ES705_SYNC_POLLING;
	u32 cmd_rspn = cmd;
	int match = 1;
	int retry = 0;	
	int retry_wakeup_cnt = 0;

	dev_info(es705->dev, "%s(): power_state(%d) onoff(%d) pdm_mic_dhwpt(%d)\n",
			__func__, power_state, onoff, es705->pdm_mic_dhwpt);

	if (es705->micbias_en_cb) {
		if (!power_state) { /* by esxxx_wakeup_chip, esxxx_sleep_chip */
			dev_info(es705->dev, "%s(): micbias control %d\n",
						__func__, !onoff);
			es705->micbias_en_cb(!onoff, SEC_MICBIAS_SRC_VS);
		}
	}

	if (onoff != es705->pdm_mic_dhwpt) {
		if (power_state) {
			mutex_lock(&es705->pm_mutex);			
			if (delayed_work_pending(&es705->sleep_work) ||
				(es705->pm_state == ES705_POWER_SLEEP_PENDING)) {
			...
			retry = 0;
			do {
				rc = es705_write_then_read(es705, &cmd, sizeof(cmd), &cmd_rspn, match);
				if (rc) {
					dev_info(es705->dev, "%s(): Retry sync cmd, retry(%d) rspn=0x%08x\n",
								__func__, retry, cmd_rspn);
					msleep(SYNC_DELAY);
				}
				else
					break;
...
set_pdm_mic_dhwpt:		
		if (onoff) {
			rc = es705_cmd(es705, ES705_PDM_MIC_DHWPT_BTOD);
			if (rc < 0) {
				dev_err(es705->dev, "%s(): Failed PDM DHWPT B to D", __func__);
			}

			rc = es705_cmd(es705, ES705_PDM_MIC_DHWPT_ATOC);
			if (rc < 0) {
				dev_err(es705->dev, "%s(): Failed PDM DHWPT A to C", __func__);
			}
			else
			...
			rc = es705_cmd(es705, ES705_PDM_MIC_DHWPT_RESET);
			if (rc < 0) {
				dev_err(es705->dev, "%s(): Failed Reset PDM DHWPT",	__func__);
			}
			else
				es705->pdm_mic_dhwpt = 0;
		}
		if (power_state) {
			/* Go to sleep */
			cmd = (ES705_SET_SMOOTH << 16) | ES705_SET_SMOOTH_RATE;
...
es705_pdm_mic_dhwpt_exit:
	dev_info(es705->dev, "%s(): Exit\n",__func__);
	mutex_unlock(&es705->pm_mutex);
	return rc;
}

static int es705_get_pdm_mic_dhwpt(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = es705_priv.pdm_mic_dhwpt;
	dev_dbg(es705_priv.dev, "%s(): pdm_mic_dhwpt = %d\n",
			__func__, es705_priv.pdm_mic_dhwpt);

	return 0;
}

static int es705_put_pdm_mic_dhwpt(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{	
	int rc = 0;
	int onoff = 0;

	if (es705_priv.pdm_mic_dhwpt == ucontrol->value.integer.value[0]) {
		dev_info(es705_priv.dev, "%s(): skip to set pdm_mic_dhwpt[%d->%ld]\n",
					__func__, 
					es705_priv.pdm_mic_dhwpt,
					ucontrol->value.integer.value[0]);
	} else {
		...
		rc = es705_pdm_mic_dhwpt(&es705_priv, 1, onoff);
		if (rc)
			dev_info(es705_priv.dev, "%s(): failed rc = %d\n", __func__, rc);	
	}
	return rc;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	unsigned int value;
	int rc = 0;
#ifdef FEATURE_STAND_ALONE_VS_FW
	u32 cmd_resp;
#endif
	...
	if (value == 0 || value == ES705_SET_POWER_STATE_MP_SLEEP ||
		...
		if (value == ES705_SET_POWER_STATE_SLEEP) {
			...
			rc = es705_priv.cmd(&es705_priv, ES_SET_EVENT_RESP, 0, &cmd_resp);
			if (rc)
				dev_err(es705_priv.dev, "%s(): Enable Event Intr fail\n",
				__func__);
#endif
		} else if (value == ES705_SET_POWER_STATE_VS_LOWPWR) {
			if (es705_priv.es705_power_state ==
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	fp = filp_open("/opt/usr/media/Downloads/vs_capture.bin",
...
static struct snd_kcontrol_new es705_digital_ext_snd_controls[] = {
	...
	SOC_SINGLE_EXT("ES705 Voice Wakeup Enable", SND_SOC_NOPM, 0, 2, 0,
				esxxx_get_stand_alone_vs_fw,
				esxxx_put_stand_alone_vs_fw),
	SOC_SINGLE_EXT("ES705 Update Keyword", SND_SOC_NOPM, 0, 1, 0,
				esxxx_get_update_vs_keyword,
				esxxx_put_update_vs_keyword),
#else
	SOC_SINGLE_EXT("ES705 Voice Wakeup Enable", SND_SOC_NOPM, 0, 2, 0,
	...
	SOC_SINGLE_EXT("Audience Network Type", SND_SOC_NOPM, 0, 1, 0,
...
#if defined(FEATURE_PDM_MIC_DHWPT)
	SOC_SINGLE_EXT("ES705 PDM MIC DHWPT", SND_SOC_NOPM, 0, 2, 0,
				es705_get_pdm_mic_dhwpt,
				es705_put_pdm_mic_dhwpt),
#endif
};
...
#endif
#if defined(FEATURE_STAND_ALONE_VS_FW)
#define USE_MUTEX_ON_HANDLER
#if defined(FEATURE_SENSORY_EXP) && defined(GEAR3_DEBUG)
void check_vs_status(struct es705_priv *es705)
{
	int rc = 0;
	u32 rspn = 0;
	u32 chk_vs_wordid = 0x8016E007;
	u32 chk_vs_svscore = 0x8016E008;
	u32 chk_vs_fscore = 0x8016E009;
	u32 chk_vs_noiseFloor = 0x8016E048;
	u16 vs_status_response = 0;

#ifdef USE_MUTEX_ON_HANDLER
	mutex_lock(&es705->pm_mutex);
	if (!es705->stand_alone_vs_fw_enable) {
		dev_err(es705->dev, "%s(): already disabled\n", __func__);
		goto check_vs_status_exit;
	}
#endif
	rc = es705_write_then_read(es705, &chk_vs_wordid,
			sizeof(chk_vs_wordid),	&rspn, 0);
	if (rc < 0) {
		dev_err(es705->dev, "%s(): cannot check the word ID\n",
								__func__);
	} else {
		vs_status_response = rspn & 0xFFFF;
		dev_info(es705->dev, "%s(): Word ID is %d\n",
	...
	if (rc < 0) {
		dev_err(es705->dev, "%s(): cannot check the SvScore\n",
								__func__);
	} else {
		vs_status_response = rspn & 0xFFFF;
	...
	rc = es705_write_then_read(es705, &chk_vs_fscore,
			sizeof(chk_vs_fscore), &rspn, 0);
	if (rc < 0) {
		dev_err(es705->dev, "%s(): cannot check the FinalScore\n",
								__func__);
	} else {
		vs_status_response = rspn & 0xFFFF;
	...
	if (rc < 0) {
		dev_err(es705->dev, "%s(): cannot check the noiseFloor\n",
								__func__);
	} else {
		vs_status_response = rspn & 0xFFFF;
...
#ifdef USE_MUTEX_ON_HANDLER
check_vs_status_exit:
	mutex_unlock(&es705->pm_mutex);
#endif /* USE_MUTEX_ON_HANDLER */
}
#endif
static void es705_event_status(struct es705_priv *es705)
{
	...
	if ((event_response & 0x00FF) == KW_DETECTED) {
		...
		/* Select streaming endpoint,
				moved from middleware to kernel space */
		rc = es705_cmd(es705, ES_SELECT_STREAMING);
		if (rc < 0) {
			dev_err(es705->dev, "%s(): ES_SELECT_STREAMING Failed",	__func__);
		}
...
#if defined(FEATURE_STAND_ALONE_VS_FW)
	const char *fw_filename = "audience-es705-fw-ponte.bin";
#else
...
static __init int es705_init(void)
{
	int rc = 0;
#ifdef CONFIG_MACH_B3
	if (!check_es705_support(system_rev)) {
		pr_info("This board doesn't have es705 0x%x\n", system_rev);
		return -ENODEV;
	}
#endif
