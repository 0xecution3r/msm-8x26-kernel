...
static ssize_t read_spi_data(struct dbmd2_private *p, void *buf, size_t len)
{
	struct dbmd2_spi_private *spi_p =
				(struct dbmd2_spi_private *)p->chip->pdata;
	size_t count = MAX_CMD_SEND_SIZE;
	u8 local_buf[MAX_CMD_SEND_SIZE];
	u8 *d = (u8 *)buf;
	ssize_t i;
	int ret;
...
{
	unsigned int count = MAX_CMD_SEND_SIZE;
	size_t i;
	...
	/* First word is garbage and should be dropped */
	ret = read_spi_data(p, local_buf, 2);
	if (ret < 0) {
			dev_err(p->dev,
...
static int dbmd2_spi_probe(struct spi_device *client)
{
	...
	if (cs_gpio < 0 || !gpio_is_valid(cs_gpio)) {
		cs_gpio = -1;
		dev_info(p->dev,
			"%s: spi cs-gpio missing or invalid\n", __func__);
	}
