...
#if defined(CONFIG_SND_SOC_DBMD2P3_V34_PORT)
...
#define DRIVER_VERSION				"3.018"

#define DBD2_AUDIO_MODE_PCM			0
#define DBD2_AUDIO_MODE_MU_LAW			1

#define DBMD2_SET_DVS

enum dbmd2_detection_mode {
	DETECTION_MODE_PHRASE = 1,
	DETECTION_MODE_VOICE_ENERGY,
	DETECTION_MODE_VOICE_COMMAND,
	DETECTION_MODE_PHRASE_DONT_LOAD,
	DETECTION_MODE_PHRASE_DONT_LOAD_FWD_COMPAT,
	DETECTION_MODE_MAX = DETECTION_MODE_PHRASE_DONT_LOAD_FWD_COMPAT
};
...
static void dbmd2_wakeup_set(struct dbmd2_private *p)
{
	...
	dev_dbg(p->dev, "%s\n", __func__);
#ifdef DBMD2_SET_DVS
	gpio_tlmm_config(GPIO_CFG(p->pdata->gpio_wakeup, 0, GPIO_CFG_OUTPUT,
...
static int dbmd2_check_recovery(struct dbmd2_private *p)
{
	int ret = 0;
	if (p->try_recovery) {
		p->try_recovery = false;
		p->unlock(p);
		ret = dbmd2_perform_recovery(p);
		if (ret)
			dev_err(p->dev,
				"%s: firmware has been recoverd\n",
				__func__);
		p->lock(p);
		dbmd2_wake(p);
	}
		...
		struct dbmd2_private *p, enum dbmd2_power_modes mode)
{
	...
	dev_info(p->dev, "%s: would move %s -> %s (%2.2d -> %2.2d)\n",
			__func__,
	...
	case DBMD2_PM_SLEEPING:
		...
		case DBMD2_PM_BOOTING:
			...
			if (ret) {
				dev_err(p->dev,
					"%s: failed to wake the chip up, try recovery\n",
					__func__);
				ret = dbmd2_check_recovery(p);
				if (ret) {
					dev_err(p->dev,
					"%s: Recovery fail\n",
					__func__);
...
static int dbmd2_set_mode(struct dbmd2_private *p, int mode)
{
	...
	if (mode >= 0 && mode < DBMD2_NR_OF_STATES) {
		dev_info(p->dev, "%s: mode: %d (%s)\n",
			__func__, mode, dbmd2_state_names[mode]);
	} else {
		dev_warn(p->dev, "%s: mode: %d (invalid)\n", __func__, mode);
		return -EINVAL;
	}
...
#if defined(CONFIG_SND_SOC_DBMD2P3_V34_PORT)
		if (p->micbias_en_cb)
			p->micbias_en_cb(0, SEC_MICBIAS_SRC_VS);
#endif
		break;
...
#if defined(CONFIG_SND_SOC_DBMD2P3_V34_PORT)
		if (p->micbias_en_cb)
			p->micbias_en_cb(1, SEC_MICBIAS_SRC_VS);
#endif
		break;
	...
	if (mode == DBMD2_DETECTION) {
		...
		ret = dbmd2_send_cmd(
			p, DBMD2_VA_SENS_RECOGNITION_MODE | recog_mode, NULL);
		if (ret < 0) {
			dev_err(p->dev,
				     ...
				     enum dbmd2_microphone_mode mode)
{
	...
	case DBMD2_MIC_MODE_ANALOG:
		/* restore anaog mic gain control after configuration */
		ret1 = dbmd2_send_cmd(p, DBMD2_VA_MIC_GAIN,
					&mic_tmp);
		if (ret1 < 0)
			break;
		ret1 = dbmd2_send_cmd(p,
		...
		ret2 = dbmd2_send_cmd(p, DBMD2_VA_MIC_GAIN | mic_tmp,
					NULL);
		break;
	default:
...
static int dbmd2_config_va_mode(struct dbmd2_private *p)
{
	...
	ret = dbmd2_va_set_speed(p, DBMD2_VA_SPEED_BUFFERING);
	if (ret)
		goto out;
...
static ssize_t dbmd2_va_trigger_level_show(struct device *dev,
					...
					char *buf)
{
	return dbmd2_reg_show(dev, DBMD2_VA_SENS_TG_THRESHOLD, attr, buf);
}
...
static ssize_t dbmd2_va_trigger_level_store(struct device *dev,
					 ...
					 const char *buf, size_t size)
{
	return dbmd2_reg_store(dev, DBMD2_VA_SENS_TG_THRESHOLD, attr,
			       buf, size, DBMD2_FW_VA);
}
...
static ssize_t dbmd2_va_verification_level_show(struct device *dev,
					     ...
					     char *buf)
{
	return dbmd2_reg_show(dev, DBMD2_VA_SENS_VERIF_THRESHOLD, attr, buf);
}
...
static ssize_t dbmd2_va_verification_level_store(struct device *dev,
					      ...
					      const char *buf, size_t size)
{
	return dbmd2_reg_store(dev, DBMD2_VA_SENS_VERIF_THRESHOLD, attr, buf,
			      size, DBMD2_FW_VA);
}
...
static int dbmd2_va_amodel_build_load(struct dbmd2_private *p)
{
	int ret;
...
static int dbmd2_va_amodel_update(struct dbmd2_private *p, int val)
{
	int ret;
	...
	case DETECTION_MODE_PHRASE:
		...
		break;
	case DETECTION_MODE_PHRASE_DONT_LOAD_FWD_COMPAT:
	...
	ret = dbmd2_va_amodel_build_load(p);
	if (ret < 0)
		dev_err(p->dev,
					 ...
					 const char *buf, size_t size)
{
	...
	ret = dbmd2_va_amodel_update(p, val);
...
#if defined(CONFIG_SND_SOC_DBMD2P3_V34_PORT)
static ssize_t dbmd2_trigger_info_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct dbmd2_private *p = dev_get_drvdata(dev);
	int ret;
	int i;
	u16 addr[3] = {0x56, 0x57, 0x5A};
	u16 data[3] = {0, 0, 0};
	...
	for(i = 0; i < 3; i++) {
		if (p->active_fw == DBMD2_FW_VQE)
			ret = dbmd2_send_cmd(p,
				DBMD2_VQE_SET_INDIRECT_REG_ADDR_ACCESS_CMD | (u32)addr[i],
				NULL);
		else
			ret = dbmd2_send_cmd(p,
					     DBMD2_VA_SET_D2PARAM_ADDR | (u32)addr[i],
					     NULL);
		...
		if (p->active_fw == DBMD2_FW_VQE)
			...
			ret = dbmd2_send_cmd(p, DBMD2_VA_GET_D2PARAM, &data[i]);
...
#if defined(CONFIG_SND_SOC_DBMD2P3_V34_PORT)
static DEVICE_ATTR(trigger_info, S_IRUGO | S_IWUSR,
		   dbmd2_trigger_info_show, NULL);
#endif
...
static int dbmd2_perform_recovery(struct dbmd2_private *p)
{
	...
	dev_err(p->dev, "%s: active FW - %s\n", __func__,
			dbmd2_fw_type_to_str(active_fw));

	memset(&saved_va_flags, 0, sizeof(saved_va_flags));
	if (active_fw == DBMD2_FW_VA) {
		p->va_flags.buffering = 0;
		flush_work(&p->sv_work);
		memcpy(&saved_va_flags, &(p->va_flags), sizeof(saved_va_flags));
		p->wakeup_release(p);
		ret = dbmd2_request_and_load_fw(p, 1, 0, 0);

	} else if (active_fw == DBMD2_FW_VQE) {
		p->wakeup_release(p);
		ret = dbmd2_request_and_load_fw(p, 0, 1, 0);
	} else {
		p->wakeup_release(p);
		ret = dbmd2_request_and_load_fw(p, 0, 1, 0);
	}
	...
	if (active_fw == DBMD2_FW_VA) {
		...
		if (saved_va_flags.amodel_len > 0 &&
			...
			dbmd2_acoustic_model_load(
				p, p->amodel_buf, saved_va_flags.amodel_len);
			p->lock(p);
		}
	} else if (active_fw == DBMD2_FW_VQE) {
		if (p->vqe_flags.in_call &&
	...
	dev_err(p->dev, "%s: FW has been restarted!\n", __func__);
 ...
 * ------------------------------------------------------------------------ */
#if defined(CONFIG_SND_SOC_DBMD2P3_V34_PORT)
int dbmd2_register_mic_bias_en_cb(void (*micbias_en_cb_fn)(int en, int src))
{
	struct dbmd2_private *p = dbd2_data;
	int ret = 0;

	if (!dbd2_data) {
		dev_err(p->dev, "%s(): too early\n", __func__);
		return -ENODEV;
	}

	if (micbias_en_cb_fn) {
		dbd2_data->micbias_en_cb = micbias_en_cb_fn;
		dev_info(p->dev, "%s(): micbias_en_cb registered 0x%p\n",
					__func__, micbias_en_cb_fn);
	} else {
		dbd2_data->micbias_en_cb = NULL;
		ret = -EINVAL;
		dev_err(p->dev, "%s(): NULL micbias_en_cb_fn\n", __func__);
	}

	return ret;
}
EXPORT_SYMBOL(dbmd2_register_mic_bias_en_cb);
#endif
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	return dbmd2_va_amodel_update(p, value);
}
...
#if defined(CONFIG_SND_SOC_DBMD2P3_V34_PORT)
static int dbmd2_va_trigger_info_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct dbmd2_private *p = dbd2_data;
	int ret, i;
	...
	for (i = 0; i < 3; i++) {
		...
		ucontrol->value.integer.value[i] = data[i];
	}
	p->unlock(p);
...
static const struct snd_kcontrol_new dbmd2_snd_controls[] = {
	...
	SOC_SINGLE_MULTI_EXT("Trigger info", SND_SOC_NOPM, 0, 65535, 0, 3,
				dbmd2_va_trigger_info_get, NULL),
#endif
};
...
#if defined(CONFIG_SND_SOC_DBMD2P3_V34_PORT)
static void orbis_wc1l_trigger(void)
{
	struct dbmd2_private *p = dbd2_data;
...
static void dbmd2_uevent_work(struct work_struct *work)
{
	...
	if (p->va_detection_mode == DETECTION_MODE_VOICE_ENERGY) {
		...
		ret = dbmd2_send_cmd(
				p, DBMD2_VA_SENS_ALTWORDID, &event_id);
		...
		ret = dbmd2_send_cmd(
				p, DBMD2_VA_SENS_WORDID, &event_id);
...
#if defined(CONFIG_SND_SOC_DBMD2P3_V34_PORT)
	{
		...
		ret = dbmd2_send_cmd(p,
				DBMD2_VA_SET_D2PARAM_ADDR | (u32) 0x5d, NULL);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: failed set final_score addr\n", __func__);
			p->unlock(p);
			return;
		}
		ret = dbmd2_send_cmd(p,
				DBMD2_VA_GET_D2PARAM, &final_score);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: failed get final_score\n", __func__);
			p->unlock(p);
			return;
		} else {
			dev_err(p->dev,
				"%s: final score %u\n", __func__, final_score);
		}

		ret = dbmd2_send_cmd(p,
				DBMD2_VA_SET_D2PARAM_ADDR | (u32) 0x5e, NULL);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: failed set sv_score addr\n", __func__);
			p->unlock(p);
			return;
		}
		ret = dbmd2_send_cmd(p,
				DBMD2_VA_GET_D2PARAM, &sv_score);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: failed get sv_score\n", __func__);
			p->unlock(p);
			return;
		} else {
			dev_err(p->dev,
		...
		ret = dbmd2_send_cmd(p,
				DBMD2_VA_SET_D2PARAM_ADDR | (u32) 0x61, NULL);
		if (ret < 0) {
			dev_err(p->dev,
			"%s: failed set noise floor addr\n", __func__);
			p->unlock(p);
			return;
		}
		ret = dbmd2_send_cmd(p,
				DBMD2_VA_GET_D2PARAM, &noise_floor);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: failed get noise floor\n", __func__);
			p->unlock(p);
			return;
		} else {
			dev_err(p->dev,
				"%s: noise floor %u\n",
						__func__, noise_floor);
		}
		p->unlock(p);
	}
#endif
...
static int dbmd2_common_probe(struct dbmd2_private *p)
{
	...
	if (init_input_device() < 0)
		...
		dbmd2_remote_register_event_callback((event_cb)orbis_wc1l_trigger);
#endif
...
#if defined(CONFIG_SND_SOC_DBMD2P3_V34_PORT)
	p->micbias_en_cb = NULL; /* initial value */
#endif
