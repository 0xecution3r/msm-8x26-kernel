...
#endif
#if defined(CONFIG_SND_SOC_DBMD2P3) && !defined(CONFIG_SND_DSPG_DBMD2)
...
static int tapan_main_mic_mute_put(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	int mute = ucontrol->value.integer.value[0];
	unsigned char tx1_cfg;
	unsigned char tx2_cfg;
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (mute) {
		/* analog mic */
		tx1_cfg = snd_soc_read(codec, TAPAN_A_CDC_TX1_VOL_CTL_CFG);
		snd_soc_write(codec,
			TAPAN_A_CDC_TX1_VOL_CTL_CFG, tx1_cfg | 0x01);
		tx1_cfg = snd_soc_read(codec, TAPAN_A_CDC_TX1_VOL_CTL_CFG);
		/* digital mic */
		tx2_cfg = snd_soc_read(codec, TAPAN_A_CDC_TX2_VOL_CTL_CFG);
		snd_soc_write(codec,
			TAPAN_A_CDC_TX2_VOL_CTL_CFG, tx2_cfg | 0x01);
		tx2_cfg = snd_soc_read(codec, TAPAN_A_CDC_TX2_VOL_CTL_CFG);
	} else {
		/* analog mic */
		tx1_cfg = snd_soc_read(codec, TAPAN_A_CDC_TX1_VOL_CTL_CFG);
		snd_soc_write(codec,
			TAPAN_A_CDC_TX1_VOL_CTL_CFG, tx1_cfg & 0xfe);
		tx1_cfg = snd_soc_read(codec, TAPAN_A_CDC_TX1_VOL_CTL_CFG);
		/* digital mic */
		tx2_cfg = snd_soc_read(codec, TAPAN_A_CDC_TX2_VOL_CTL_CFG);
		snd_soc_write(codec,
			TAPAN_A_CDC_TX2_VOL_CTL_CFG, tx2_cfg & 0xfe);
		tx2_cfg = snd_soc_read(codec, TAPAN_A_CDC_TX2_VOL_CTL_CFG);
	}

	pr_info("%s: mute=%d, reg tx1=0x%x, tx2=0x%x\n", __func__,
				mute, tx1_cfg, tx2_cfg);

	return 0;
}

static int tapan_main_mic_mute_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	unsigned char tx1_cfg, tx2_cfg;
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	tx1_cfg = snd_soc_read(codec, TAPAN_A_CDC_TX1_VOL_CTL_CFG);
	tx2_cfg = snd_soc_read(codec, TAPAN_A_CDC_TX2_VOL_CTL_CFG);
	pr_info("%s: mute tx1=0x%x, tx2=-0x%x\n", __func__,
						tx1_cfg, tx2_cfg);

	/* assume user needs dmic mute only */
	ucontrol->value.integer.value[0] = tx2_cfg & 0x01;
...
static const struct snd_kcontrol_new tapan_common_snd_controls[] = {
	...
	SOC_SINGLE_EXT("HPHR Impedance", 0, 1, UINT_MAX, 0,
		       tapan_hph_impedance_get, NULL),
#endif
	SOC_SINGLE_EXT("Main Mic Mute", SND_SOC_NOPM, 0, 1, 0,
			tapan_main_mic_mute_get, tapan_main_mic_mute_put),
};
...
static const struct snd_soc_dapm_route wcd9306_map[] = {
	...
	/* External LDO for Main Mic Bias */
	{"External LDO", NULL, "LDO_H"},
};
...
static int tapan_es705_startup(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
...
static void tapan_es705_shutdown(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
...
static int tapan_es705_hw_params(struct snd_pcm_substream *substream,
		struct snd_pcm_hw_params *params,
		struct snd_soc_dai *dai)
{
	int rc = 0;
...
static int tapan_es705_set_channel_map(struct snd_soc_dai *dai,
				unsigned int tx_num, unsigned int *tx_slot,
				unsigned int rx_num, unsigned int *rx_slot)
...
static int tapan_es705_get_channel_map(struct snd_soc_dai *dai,
				unsigned int *tx_num, unsigned int *tx_slot,
				unsigned int *rx_num, unsigned int *rx_slot)
...
static struct snd_soc_dai_ops tapan_es705_dai_ops = {
	.startup = tapan_es705_startup, /* tapan_startup, */
	.shutdown = tapan_es705_shutdown, /* tapan_shutdown, */
/* TODO: check tapan driver version
	.prepare = tapan_prepare,
*/
	.hw_params = tapan_es705_hw_params, /* tapan_hw_params, */
	.set_sysclk = tapan_set_dai_sysclk,
	.set_fmt = tapan_set_dai_fmt,
	.set_channel_map = tapan_es705_set_channel_map,
			/* tapan_set_channel_map, */
	.get_channel_map = tapan_es705_get_channel_map,
...
#if defined(CONFIG_SND_SOC_ES705) && !defined(CONFIG_SND_SOC_ES705_STAND_ALONE_VS_FW)
...
static const struct snd_soc_dapm_widget tapan_9306_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MICBIAS_E("MIC BIAS3 Internal2", SND_SOC_NOPM, 7, 0,
		tapan_codec_enable_micbias, SND_SOC_DAPM_PRE_PMU |
		SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MICBIAS_E("External LDO", 0, 0, 0,
...
static int __devinit tapan_probe(struct platform_device *pdev)
{
	...
	if (!is_wcd9306) {
		if (wcd9xxx_get_intf_type() == WCD9XXX_INTERFACE_TYPE_SLIMBUS)
		{
			...
			if (check_es705_support(system_rev)) {
				int i;
				for (i = 0; i < ARRAY_SIZE(tapan_dai); i++)
					tapan_dai[i].ops = &tapan_es705_dai_ops;
			}
#endif
