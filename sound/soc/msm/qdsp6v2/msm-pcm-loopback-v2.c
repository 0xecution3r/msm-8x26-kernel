...
enum {
   LOOPBACK_SESSION_1_INDEX,
   LOOPBACK_SESSION_2_INDEX,
   LOOPBACK_SESSION_MAX_INDEX,
};
...
struct loopback_info {
   struct msm_pcm_loopback loop[LOOPBACK_SESSION_MAX_INDEX];
};
...
static int msm_pcm_open(struct snd_pcm_substream *substream)
{
	...
	struct asm_stream_mtmx_strtr_param_window_v2_t asm_mtmx_strrt_window;

	dev_dbg(rtd->platform->dev, "%s stream name: %s\n", __func__,
		substream->pcm->id);

	loopback_info = dev_get_drvdata(rtd->platform->dev);
	if (!strncmp("MultiMedia6", substream->pcm->id, 11))
		pcm = &loopback_info->loop[LOOPBACK_SESSION_1_INDEX];
	else
		pcm = &loopback_info->loop[LOOPBACK_SESSION_2_INDEX];
	...
	if (pcm->instance == 2) {
		...
		dev_dbg(rtd->platform->dev,
                       "%s: Calling render window start\n", __func__);
		asm_mtmx_strrt_window.window_lsw = 0x00000000;
		asm_mtmx_strrt_window.window_msw = 0x80000000;
		q6asm_send_mtmx_strtr_window_start(pcm->audio_client,
						&asm_mtmx_strrt_window);
		dev_dbg(rtd->platform->dev,
			"%s: Calling render window end\n", __func__);
		asm_mtmx_strrt_window.window_lsw = 0xffffffff;
		asm_mtmx_strrt_window.window_msw = 0x7fffffff;
		q6asm_send_mtmx_strtr_window_end(pcm->audio_client,
						&asm_mtmx_strrt_window);
	}
			     ...
			     struct snd_pcm_hw_params *params)
{
	...
	return 0;
/* removing unncessesory buffer allocation 2014.08.15
...
static __devinit int msm_pcm_probe(struct platform_device *pdev)
{
	...
	if (!loopback) {
		dev_err(&pdev->dev, "%s No memory for loopback info\n",
			__func__);
		return -ENOMEM;
	}

	for (i = 0; i < LOOPBACK_SESSION_MAX_INDEX; i++) {
