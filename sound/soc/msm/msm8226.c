...
#endif
#if defined(CONFIG_SND_SOC_DBMD2)
#include "../codecs/dbmd2/dbmd2-export.h"
#elif defined(CONFIG_SND_SOC_DBMD2P3)
...
#ifdef CONFIG_SND_SOC_TAPAN_MBHC_HSDET
...
#if defined(CONFIG_MACH_B3) || defined(CONFIG_MACH_WJ1) || defined(CONFIG_MACH_WC1) \
	||defined(CONFIG_MACH_NC1)
static int micbias_en_gpio;
static int mainmic_stable_delay;
static int micbias_forced_enable;
#endif
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	|| defined(CONFIG_MACH_WC1) ||defined(CONFIG_MACH_NC1)
void micbias_external_ldo_control(int enable, int bias_src)
{
	static int bias_control;
	static int prev_bias_control = -1;

	pr_debug("%s : request %s from src 0x%x\n", __func__,
					enable ? "ON" : "OFF", bias_src);

	if (enable)
		bias_control |= bias_src;
	else
		bias_control &= ~bias_src;

	pr_info("%s : result %s (status 0x%x)\n", __func__,
				bias_control ? "ON" : "OFF", bias_control);

	if (micbias_en_gpio > 0) {
		if (micbias_forced_enable && !bias_control) {
			pr_debug("%s(): skip micbias off - force enable\n",
					__func__);
		} else {
			if (bias_control == prev_bias_control) {
				pr_debug("%s: skip gpio ctrl - same value\n",
								__func__);
			} else {
				gpio_set_value_cansleep(micbias_en_gpio,
							bias_control ? 1 : 0);
			}
		}
	} else {
		pr_debug("%s: skip gpio ctrl", __func__);
	}
	prev_bias_control = bias_control;
}
		...
		struct snd_kcontrol *k, int event)
{
	...
	pr_info("%s : Event %d,  SND_SOC_DAPM_EVENT_ON:%d\n",
			__func__, (event), SND_SOC_DAPM_EVENT_ON(event));

	micbias_external_ldo_control(SND_SOC_DAPM_EVENT_ON(event),
					SEC_MICBIAS_SRC_CODEC);

	if (event == SND_SOC_DAPM_PRE_PMU) {
		if (mainmic_stable_delay) {
			pr_info("%s: delay %d msec\n", __func__,
						mainmic_stable_delay);
			if (mainmic_stable_delay <= 20) {
				usleep_range(mainmic_stable_delay * 1000,
					mainmic_stable_delay * 1000 + 100);
			} else {
				msleep_interruptible(mainmic_stable_delay);
			}
		}
	}
	return 0;
...
#if defined(CONFIG_MACH_B3) || defined(CONFIG_MACH_WJ1)\
	|| defined(CONFIG_MACH_WC1) || defined(CONFIG_MACH_NC1)
static const char *const onoff_text[] = {"Off", "On", "Unknown"};
#endif
...
#if defined(CONFIG_MACH_B3) || defined(CONFIG_MACH_WJ1)\
	|| defined(CONFIG_MACH_WC1) || defined(CONFIG_MACH_NC1)
static int sec_mainmic_stable_delay_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s: mainmic delay %d\n",
		__func__, mainmic_stable_delay);
	ucontrol->value.integer.value[0] = mainmic_stable_delay;
	return 0;
}

static int sec_mainmic_stable_delay_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	mainmic_stable_delay = ucontrol->value.integer.value[0];
	pr_info("%s: mainmic delay %d",
		__func__, mainmic_stable_delay);
	return 1;
}

static int sec_mainmic_bias_get(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
	int value;

	if (micbias_en_gpio > 0) {
		value = gpio_get_value_cansleep(micbias_en_gpio);
	} else {
		pr_info("%s: micbias_en_gpio is not assigned\n", __func__);
		value = 2; /* UNKNOWN */
	}
	pr_info("%s: mainmic bias %d (%d)\n", __func__,
				value, micbias_forced_enable);
	ucontrol->value.integer.value[0] = value;
	return 0;
}

static int sec_mainmic_bias_put(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
	int value = ucontrol->value.integer.value[0];
	if (value == 2) {
		pr_warn("%s(): set micbias to unknown is not supported\n",
				__func__);
		return 1;
	}
	if (micbias_en_gpio > 0) {
		pr_info("%s: mainmic bias %d", __func__, value);
		gpio_set_value_cansleep(micbias_en_gpio, value);
		micbias_forced_enable = value;
	} else {
		pr_info("%s: skip micbias force control\n", __func__);
	}
	return 1;
}
...
#if defined(CONFIG_MACH_B3) || defined(CONFIG_MACH_WJ1)\
	|| defined(CONFIG_MACH_WC1) || defined(CONFIG_MACH_NC1)
	SOC_ENUM_SINGLE_EXT(3, onoff_text),
#endif
};
...
#if defined(CONFIG_MACH_B3) || defined(CONFIG_MACH_WJ1)\
	|| defined(CONFIG_MACH_WC1) || defined(CONFIG_MACH_NC1)
	/* "Main Mic Stable" is msec value. */
	SOC_SINGLE_EXT("Main Mic Stable", SND_SOC_NOPM, 0, 1000, 0,
		sec_mainmic_stable_delay_get, sec_mainmic_stable_delay_put),

	SOC_ENUM_EXT("Main Mic Bias", msm_snd_enum[5],
			sec_mainmic_bias_get, sec_mainmic_bias_put),
#endif
...
#if defined(CONFIG_MACH_B3) || defined(CONFIG_MACH_WJ1)\
	|| defined(CONFIG_MACH_WC1) || defined(CONFIG_MACH_NC1)
static int msm_prepare_external_ldo(struct snd_soc_card *card)
{
	int ret;
	if (micbias_en_gpio >= 0) {
		dev_dbg(card->dev, "%s : micbias en msm gpio request %d",
				__func__, micbias_en_gpio);
		ret = gpio_request(micbias_en_gpio, "MICBIAS_TO_CODEC");
		if (ret) {
			dev_err(card->dev,
				"%s: Failed to request external ldo gpio %d error %d\n",
					__func__, micbias_en_gpio, ret);
			return ret;
		}
		gpio_direction_output(micbias_en_gpio, 0);
	}
	return 0;
}
#endif
...
static int tapan_ready;
int msm8226_get_tapan_ready(void)
{
	return tapan_ready;
}
EXPORT_SYMBOL(msm8226_get_tapan_ready);

void msm8226_set_tapan_ready(int ready)
{
	tapan_ready = !!ready;
}
#endif
...
#if defined(CONFIG_MACH_B3) || defined(CONFIG_MACH_WJ1)\
	|| defined(CONFIG_MACH_WC1) || defined(CONFIG_MACH_NC1)
	micbias_en_gpio = of_get_named_gpio(pdev->dev.of_node,
				"qcom,micbias-en-gpio", 0);
	if (micbias_en_gpio < 0) {
		dev_warn(&pdev->dev,
			"Looking up %s property in node %s failed %d\n",
			"qcom,micbias-en-gpio", pdev->dev.of_node->full_name,
			micbias_en_gpio);
	} else {
		dev_info(&pdev->dev, "micbias_en_gpio %d\n", micbias_en_gpio);

		ret = msm_prepare_external_ldo(card);
		if (ret) {
			dev_err(&pdev->dev,
				"msm_prepare_external_ldo() failed %d\n", ret);
			gpio_free(micbias_en_gpio);
			micbias_en_gpio = 0;
		}
	}
#endif
...
#if defined(CONFIG_SND_SOC_ES705)
	if (micbias_en_gpio > 0) {
		ret = es705_register_mic_bias_en_cb(
				micbias_external_ldo_control);
		if (ret < 0)
			pr_err("%s: es705_register_mic_bias_en_cb failed\n",
								__func__);
		else
			pr_info("%s: es705_register_mic_bias_en_cb success\n",
								__func__); 
	} else {
		pr_info("%s: this board doesn't have ext. micbias ldo",
								__func__);
	}
#endif
#if defined(CONFIG_SND_SOC_DBMD2) || defined(CONFIG_SND_SOC_DBMD2P3)
	msm8226_set_tapan_ready(1);
	pr_err("%s(): driver_deferred_probe_trigger!!\n", __func__);
	driver_deferred_probe_trigger();
#if defined(CONFIG_SND_SOC_DBMD2_V34_PORT) || \
			defined(CONFIG_SND_SOC_DBMD2P3_V34_PORT)
	if (micbias_en_gpio > 0) {
		ret = dbmd2_register_mic_bias_en_cb(
				micbias_external_ldo_control);
		if (ret < 0)
			pr_err("%s: dbmd2_register_mic_bias_en_cb failed\n",
								__func__);
		else
			pr_info("%s: dbmd2_register_mic_bias_en_cb success\n",
								__func__);
	} else {
		pr_info("%s: this board doesn't have ext. micbias ldo\n",
								__func__);
...
err1:
	...
	|| defined(CONFIG_MACH_WC1) || defined(CONFIG_MACH_NC1)
	if (micbias_en_gpio > 0) {
		dev_info(&pdev->dev, "%s free gpio %d\n",
			__func__, micbias_en_gpio);
		gpio_free(micbias_en_gpio);
		micbias_en_gpio = 0;
	}
