...
#ifdef CONFIG_TIZEN_WIP
#define TIZEN_L2CAP_ENC_TIMEOUT         msecs_to_jiffies(3000)
#endif
...
static inline void l2cap_check_encryption(struct l2cap_chan *chan, u8 encrypt)
{
	...
	if (encrypt == 0x00) {
		if (chan->sec_level == BT_SECURITY_HIGH)
			__set_chan_timer(chan, TIZEN_L2CAP_ENC_TIMEOUT);
		else if (chan->sec_level == BT_SECURITY_MEDIUM)
			__set_chan_timer(chan, L2CAP_ENC_TIMEOUT);
		else if (chan->sec_level == BT_SECURITY_FIPS)
