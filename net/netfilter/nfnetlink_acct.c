		   ...
		   int event, struct nf_acct *acct)
{
	...
	if (acct->flags & NFACCT_F_QUOTA) {
		...
		NLA_PUT_BE32(skb, NFACCT_FLAGS, htonl(old_flags));
...
nfacct_filter_alloc(const struct nlattr * const attr)
{
	...
	struct nlattr *tb[__NFACCT_FILTER_ATTR_MAX + 1];
	int err;

	err = nla_parse_nested(tb, __NFACCT_FILTER_ATTR_MAX, attr, filter_policy);
	if (err < 0)
		return ERR_PTR(err);
	...
	filter->mask = ntohl(nla_get_be32(tb[NFACCT_FILTER_ATTR_MASK]));
	filter->value = ntohl(nla_get_be32(tb[NFACCT_FILTER_ATTR_VALUE]));
