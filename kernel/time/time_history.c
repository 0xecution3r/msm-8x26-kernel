		...
		const struct alarm *alarm, void *caller)
{
...
#ifdef CONFIG_TIME_HISTORY_LOG_FILTER
	if (time_history_check_log_filter(&alarm->node) != 0)
		return 0;
#endif
	...
	entry->newtime = ktime_to_timespec(alarm->expires);
	entry->caller = caller;
	entry->alarmid = (void*)&alarm->node;
...
void __time_history_alarm_init(const struct alarm *alarm, void *caller)
{
	...
	alarm_info->alarmid = (void*)(&alarm->node);
...
{
	if (time_history_get_alarm_by_id(&alarm->node) == NULL)
		return;
